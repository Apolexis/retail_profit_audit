...
Для весового товара в общем справочнике сохраняется код **`fraction`** из схемы товара Эвотор. В интерфейсе номенклатуры, остатков, инвентаризации, заявок и печати он всегда подписывается как **«кг»**. Преобразование относится только к отображению и к внутренним строкам пересчёта/заявок; будущий outbound-контур не реализован и не делает вызовов записи.
