...
function normalizeCommitOptions(value: unknown): ImportCommitOptions {
  ...
  const linkNames = Array.isArray(source.linkNames) ? source.linkNames : [];
  const excludedRowIndexes = Array.isArray(source.excludedRowIndexes) ? source.excludedRowIndexes : [];
  if (categorySelections.length > 3000 || priceEdits.length > 36_000 || priceAdditions.length > 36_000 || priceRemovals.length > 36_000 || rowEdits.length > 3000 || metadataEdits.length > 3000 || productLinks.length > 3000 || linkNames.length > 3000 || excludedRowIndexes.length > 3000) {
  ...
  return {
    ...
    linkNames: linkNames.map(item => {
      if (!item || typeof item !== "object") throw new Error("Передано некорректное имя связи для строки прайс‑листа.");
      const sourceLink = item as Record<string, unknown>;
      const rowIndex = Number(sourceLink.rowIndex);
      const canonicalName = typeof sourceLink.canonicalName === "string" ? sourceLink.canonicalName.trim().replace(/\s+/g, " ") : "";
      if (!Number.isInteger(rowIndex) || rowIndex < 0 || canonicalName.length < 2 || canonicalName.length > 255) throw new Error("Передано некорректное имя связи для строки прайс‑листа.");
      return { rowIndex, canonicalName };
    }),
