...
export async function listPriceControlData() {
  ...
  const previewMap = new Map(imports.map(item => [item.id, { importId: item.id, rows: [] as Array<{ rowId: number; rawName: string; rawCategory: string | null; rawPackaging: string | null; manufacturer: string | null; placeContents: string | null; productName: string | null; linkGroupName: string | null; linkGroupCode: string | null; priceAmount: number | null }> }]));
  ...
  rows.forEach(row => {
    ...
    if (preview) preview.rows.push({ rowId: row.rowId, rawName: row.rawName, rawCategory: row.rawCategory, rawPackaging: row.rawPackaging, manufacturer: row.manufacturer, placeContents: row.placeContents, productName: row.productName, linkGroupName: row.linkGroupName, linkGroupCode: row.linkGroupCode, priceAmount: row.priceAmount === null ? null : Number(row.priceAmount) });
  });
...
/** Removes only the comparison membership; the product, offers, and price history remain intact. */
export async function unassignPriceProductLinkGroup(productId: number) {
  const db = await getDb(); if (!db) throw new Error("База данных недоступна");
  const [product] = await db.select({ id: priceProducts.id, linkGroupId: priceProducts.linkGroupId }).from(priceProducts).where(eq(priceProducts.id, productId)).limit(1);
  if (!product) throw new Error("Товар прайс‑контроля не найден.");
  if (product.linkGroupId === null) throw new Error("Товар уже не состоит в имени связи.");
  await db.update(priceProducts).set({ linkGroupId: null }).where(eq(priceProducts.id, productId));
  const [updated] = await db.select().from(priceProducts).where(eq(priceProducts.id, productId)).limit(1);
  if (!updated) throw new Error("Товар прайс‑контроля не найден.");
  return updated;
}
