...
export const inventoryRegistryRouter = router({
  ...
  createCatalogProduct: protectedProcedure.input(z.object({ canonicalName: z.string().trim().min(1).max(512), evotorCategoryName: z.string().trim().max(512).nullable().optional(), baseUnit: inventoryUnit, vatRate: z.enum(["VAT_10", "VAT_22"]).optional(), internalCostPrice: z.number().finite().min(0).max(10_000_000).nullable().optional(), markingCategory: markingCategory.optional(), alcoholCode: z.string().trim().max(255).nullable().optional(), alcoholStrengthPercent: z.number().finite().min(0).max(100).nullable().optional(), manualBarcodes: z.string().max(4_000).nullable().optional(), isVisibleInRequests: z.boolean().optional(), isEvotorExportEnabled: z.boolean().optional() })).mutation(async ({ ctx, input }) => {
    ...
    await recordChange({ actorId: actor.id, action: "operational_catalog.manual.create", entityType: "operational_catalog_product", entityId: String(after.id), afterState: { product: after.canonicalName, category: after.evotorCategoryName, unit: after.baseUnit, vatRate: after.vatRate, markingCategory: after.markingCategory, alcoholCode: after.alcoholCode, alcoholStrengthPercent: after.alcoholStrengthPercent, manualBarcodes: after.manualBarcodes, isVisibleInRequests: after.isVisibleInRequests, isEvotorExportEnabled: after.isEvotorExportEnabled, internalCostPrice: after.internalCostPrice } });
    return after;
  }),
  updateCatalogProduct: protectedProcedure.input(z.object({ id: z.number().int().positive(), canonicalName: z.string().trim().min(1).max(512), evotorCategoryName: z.string().trim().max(512).nullable().optional(), baseUnit: inventoryUnit, vatRate: z.enum(["VAT_10", "VAT_22"]), markingCategory, alcoholCode: z.string().trim().max(255).nullable().optional(), alcoholStrengthPercent: z.number().finite().min(0).max(100).nullable().optional(), manualBarcodes: z.string().max(4_000).nullable().optional(), isVisibleInRequests: z.boolean(), isEvotorExportEnabled: z.boolean() })).mutation(async ({ ctx, input }) => {
    const actor = await localActor(ctx.user.openId);
    ...
    await recordChange({ actorId: actor.id, action: "operational_catalog.update", entityType: "operational_catalog_product", entityId: String(input.id), beforeState: { product: result.before.canonicalName, category: result.before.evotorCategoryName, unit: result.before.baseUnit, vatRate: result.before.vatRate, markingCategory: result.before.markingCategory, alcoholCode: result.before.alcoholCode, alcoholStrengthPercent: result.before.alcoholStrengthPercent, manualBarcodes: result.before.manualBarcodes, isVisibleInRequests: result.before.isVisibleInRequests, isEvotorExportEnabled: result.before.isEvotorExportEnabled }, afterState: { product: result.after.canonicalName, category: result.after.evotorCategoryName, unit: result.after.baseUnit, vatRate: result.after.vatRate, markingCategory: result.after.markingCategory, alcoholCode: result.after.alcoholCode, alcoholStrengthPercent: result.after.alcoholStrengthPercent, manualBarcodes: result.after.manualBarcodes, isVisibleInRequests: result.after.isVisibleInRequests, isEvotorExportEnabled: result.after.isEvotorExportEnabled } });
    return result;
  }),
