...
export const priceControlRouter = router({
  ...
  createProduct: protectedProcedure.input(z.object({ canonicalName: z.string().trim().min(2).max(255), internalCode: z.string().trim().min(3).max(64).optional(), linkGroupId: z.number().int().positive().optional(), linkGroupName: z.string().trim().min(2).max(255).optional(), categoryId: z.number().int().positive().optional(), category: z.string().trim().max(160).optional(), variantCharacteristicId: z.number().int().positive().nullable().optional(), sizeCharacteristicId: z.number().int().positive().nullable().optional(), placeContentsCharacteristicId: z.number().int().positive().nullable().optional(), variant: z.string().trim().max(255).optional(), sizeText: z.string().trim().max(120).optional(), placeContents: z.string().trim().max(160).optional(), baseUnit: z.enum(["kg", "l", "piece", "unknown"]).optional(), defaultWeightGrams: z.number().positive().max(1_000_000).optional(), defaultVolumeMl: z.number().positive().max(1_000_000).optional(), isActive: z.boolean().optional() })).mutation(async ({ ctx, input }) => {
  ...
  updateProduct: protectedProcedure.input(z.object({ id: z.number().int().positive(), canonicalName: z.string().trim().min(2).max(255), internalCode: z.string().trim().min(3).max(64), linkGroupId: z.number().int().positive().optional(), linkGroupName: z.string().trim().min(2).max(255).nullable().optional(), categoryId: z.number().int().positive().nullable().optional(), category: z.string().trim().max(160).nullable().optional(), variantCharacteristicId: z.number().int().positive().nullable().optional(), sizeCharacteristicId: z.number().int().positive().nullable().optional(), placeContentsCharacteristicId: z.number().int().positive().nullable().optional(), variant: z.string().trim().max(255).nullable().optional(), sizeText: z.string().trim().max(120).nullable().optional(), placeContents: z.string().trim().max(160).nullable().optional(), baseUnit: z.enum(["kg", "l", "piece", "unknown"]).optional(), isActive: z.boolean().optional() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
  ...
  deleteLinkGroup: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId); const before = await deletePriceLinkGroup(input.id);
    await recordChange({ actorId: actor.id, action: "price_link_group.delete", entityType: "price_link_group", entityId: String(input.id), beforeState: before, afterState: { deleted: true } });
    return { success: true };
  }),
  ...
  bulkSetLinkGroupActive: protectedProcedure.input(z.object({ linkGroupIds: z.array(z.number().int().positive()).min(1).max(300), isActive: z.boolean() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId);
    const before = await Promise.all(input.linkGroupIds.map(id => getPriceLinkGroupAuditState(id)));
    const result = await bulkSetPriceLinkGroupsActive(input);
    const after = await Promise.all(input.linkGroupIds.map(id => getPriceLinkGroupAuditState(id)));
    await recordChange({ actorId: actor.id, action: "price_link_group.visibility_bulk_update", entityType: "price_link_group", entityId: input.linkGroupIds.join(","), beforeState: { groups: before }, afterState: { groups: after, isActive: result.isActive } });
    return result;
  }),
  ...
  deleteOffer: protectedProcedure.input(z.object({ priceId: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId);
    const before = await getPriceOfferAuditState(input.priceId);
    if (!before) throw new TRPCError({ code: "NOT_FOUND", message: "Цена прайс-листа не найдена." });
    const result = await deletePriceOffer(input.priceId);
    await recordChange({ actorId: actor.id, action: "price_offer.delete", entityType: "price_offer", entityId: String(input.priceId), beforeState: before, afterState: { importRowId: result.importRowId, sourceRowKept: true } });
    return result;
  }),
  ...
  backfillLinkNames: protectedProcedure.mutation(async ({ ctx }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId);
    const result = await backfillPriceImportLinkNames({ actorId: actor.id });
    if (result.linkedRows) {
      await recordChange({
        actorId: actor.id,
        action: "price_alias.link_name_backfill",
        entityType: "price_import_row",
        entityId: `link-names:${result.linkedRows}`,
        afterState: {
          linkedRows: result.linkedRows,
          sharedNames: result.sharedNames,
