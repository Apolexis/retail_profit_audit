...
export const priceControlRouter = router({
  ...
  updateOffer: protectedProcedure.input(z.object({ priceId: z.number().int().positive(), priceAmount: z.number().positive().max(10_000_000), priceBasis: z.enum(["kg", "l", "piece", "package", "unknown"]) })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId); const result = await updatePriceOffer(input);
    await recordChange({ actorId: actor.id, action: "price_offer.update", entityType: "price_offer", entityId: String(input.priceId), afterState: { priceAmount: input.priceAmount, priceBasis: input.priceBasis, normalized: result.normalized } });
    return result;
  }),
  updateImportDate: protectedProcedure.input(z.object({ importId: z.number().int().positive(), sourceDate: z.string().regex(/^20\d{2}-\d{2}-\d{2}$/).nullable() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
    const actor = await localActor(ctx.user.openId); const record = await updatePriceImportDate(input);
    await recordChange({ actorId: actor.id, action: "price_import.date_update", entityType: "price_import", entityId: String(input.importId), afterState: { sourceDate: record.sourceDate } });
    return record;
  }),
