...
export const priceControlRouter = router({
  ...
  updateProduct: protectedProcedure.input(z.object({ id: z.number().int().positive(), canonicalName: z.string().trim().min(2).max(255), internalCode: z.string().trim().min(3).max(64), linkGroupId: z.number().int().positive().optional(), categoryId: z.number().int().positive().nullable().optional(), category: z.string().trim().max(160).nullable().optional(), variantCharacteristicId: z.number().int().positive().nullable().optional(), sizeCharacteristicId: z.number().int().positive().nullable().optional(), placeContentsCharacteristicId: z.number().int().positive().nullable().optional(), variant: z.string().trim().max(255).nullable().optional(), sizeText: z.string().trim().max(120).nullable().optional(), placeContents: z.string().trim().max(160).nullable().optional(), baseUnit: z.enum(["kg", "l", "piece", "unknown"]).optional(), isActive: z.boolean().optional() })).mutation(async ({ ctx, input }) => {
    await requirePricePermission(ctx.user.openId, "edit");
