...
import { accountDisplayName, assertAccountCanBeDeleted, assertAdminCanResetOtherPassword, isRussianMobileUsername, normalizeLocalUsername, verifyBootstrapPassword } from "./localAuth";
...
describe("визуальная идентификация учетной записи", () => {
  it("сохраняет телефонный логин и строит имя только из непустых полей", () => {
    expect(accountDisplayName("79537573636", "Алексей", "Иванов")).toBe("Алексей Иванов");
    expect(accountDisplayName("79537573636", "", "")).toBe("+7 (953) 757-36-36");
    expect(accountDisplayName("склад_01", null, null)).toBe("склад_01");
  });
});
