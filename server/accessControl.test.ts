...
import { isAccessLevelAllowed, isImportAccessAllowed, isOperationalSignalRecipient } from "./accessControl";
...
describe("получатели операционного сигнала точки", () => {
  it("дает дедлайн только назначенному продавцу, не создавая административную рассылку", () => {
    expect(isOperationalSignalRecipient("seller")).toBe(true);
    expect(isOperationalSignalRecipient("analyst")).toBe(false);
    expect(isOperationalSignalRecipient("admin")).toBe(false);
    expect(isOperationalSignalRecipient("manager")).toBe(false);
  });
});
