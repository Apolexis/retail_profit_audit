...
OUT_DIR = Path("/home/ubuntu/retail_profit_audit_2026/audit_2026/audit_august_clean")
OUT_DIR.mkdir(parents=True, exist_ok=True)

MONTHS = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август"]
EXCLUDED_STORES = {"КА1", "АЛА", "МОЛ"}
METRIC_COLUMNS = {
    "purchases": "G",
    "revenue": "H",
    "gross_profit": "L",
...
def main():
    ...
    all_store_names = formulas_wb.sheetnames[3:]
    excluded_store_names = [store for store in all_store_names if store in EXCLUDED_STORES or store.startswith("РЕЗ")]
    store_names = [store for store in all_store_names if store not in excluded_store_names]
    ...
    for store in store_names:
        ...
        if len(blocks) != len(MONTHS):
            issue(
                ...
                f"Найдено {len(blocks)} из {len(MONTHS)} ожидаемых месячных блоков.",
                "Результат магазина за январь–август неполный или непроверяемый.",
            )
        prior_data_row = None
        store_month_rows = []
        for block in blocks:
    ...
    summary = {
        "source": str(SOURCE),
        "as_of_period": "Январь–август 2026 (период задан пользователем; год определен по имени файла)",
        ...
        "metric_definition": {
            "revenue": "Сумма строки «Продажа» (столбец H) за месячные блоки января–августа.",
            ...
            "inventory_movements": "Перемещение (R), уценка (S) и переоценка (T) отражают изменение товарного остатка и анализируются отдельно; в показатель чистой прибыли по форме AQ они не включаются.",
        },
    }
