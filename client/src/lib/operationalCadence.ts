export type CadenceMetric =
  ...
  | "expenses" | "cashExpenses" | "cashlessExpenses" | "cashTaxes" | "household" | "delivery" | "cleaning" | "bonus" | "seniority" | "supplement" | "driverCash" | "utilitiesCash" | "operatingCosts"
  ...
  /** Read-only receipt facts are overlaid from the normalized Evotor storefront. */
  | "evotorAmount" | "evotorCash" | "evotorCashless" | "evotorChecks" | "evotorAverage";
...
const cashlessCodes = ["cashless_operating_costs", "driver_cashless", "utilities_cashless", "rent", "bank_fee", "gross_profit_tax", "salary_cashless", "payroll_tax", "vacation_cashless", "vacation_tax"] as const;
...
const valuesFor = (metrics: Record<string, unknown>): CadenceValues => ({
  ...
  expenses: expenseCodes.reduce((total, key) => total + expenseAmount(metrics, key), 0), cashExpenses: cashCodes.reduce((total, key) => total + expenseAmount(metrics, key), 0), cashlessExpenses: cashlessCodes.reduce((total, key) => total + expenseAmount(metrics, key), 0), cashTaxes: expenseAmount(metrics, "personal_income_tax_22"), household: expenseAmount(metrics, "household"), delivery: expenseAmount(metrics, "delivery"), cleaning: expenseAmount(metrics, "cleaning"), bonus: expenseAmount(metrics, "bonus"), seniority: expenseAmount(metrics, "seniority"), supplement: expenseAmount(metrics, "supplement"), driverCash: expenseAmount(metrics, "driver_cash"), utilitiesCash: expenseAmount(metrics, "utilities_cash"), operatingCosts: expenseAmount(metrics, "operating_costs"),
  ...
  evotorAmount: 0, evotorCash: 0, evotorCashless: 0, evotorChecks: 0, evotorAverage: 0,
});

const cadenceKeys: CadenceMetric[] = ["revenue", "cashRevenue", "cashlessRevenue", "receiptsTotal", "grossProfit", "netProfit", "purchases", "purchaseSmoked", "purchaseFrozen", "salesSmoked", "salesFrozen", "expenses", "cashExpenses", "cashlessExpenses", "cashTaxes", "household", "delivery", "cleaning", "bonus", "seniority", "supplement", "driverCash", "utilitiesCash", "operatingCosts", "cashlessOperatingCosts", "driverCashless", "utilitiesCashless", "rent", "bankFee", "grossProfitTax", "salaryCashless", "payrollTax", "vacationCashless", "vacationTax", "salaryCash", "vacationCash", "writeoffSmoked", "writeoffFrozen", "movement", "discount", "evotorAmount", "evotorCash", "evotorCashless", "evotorChecks", "evotorAverage"];
