...
const detailLabel = (key: string, level: DetailLevel) => { const source = level === "months" ? `${key.slice(0, 7)}-01T00:00:00Z` : `${key}T12:00:00`; const date = new Date(source); if (!Number.isFinite(date.getTime())) return "Дата не указана"; return level === "days" ? date.toLocaleDateString("ru-RU", { day: "2-digit", month: "short" }) : level === "weeks" ? `Нед. ${date.toLocaleDateString("ru-RU", { day: "2-digit", month: "short" })}` : new Intl.DateTimeFormat("ru-RU", { month: "short", year: "numeric", timeZone: "UTC" }).format(date); };
...
export default function Inventory() {
  ...
  const inventoryTableSummaryLabel = trend === "stockOpen" ? "На начало" : trend === "stockClose" ? "На конец" : "Итого";
  const inventoryTableSummary = trend === "stockOpen"
    ? Number(trendData[0]?.[trend] ?? 0)
    : trend === "stockClose"
      ? Number(trendData.at(-1)?.[trend] ?? 0)
      : trendData.reduce((sum, row) => sum + Number(row[trend] ?? 0), 0);
