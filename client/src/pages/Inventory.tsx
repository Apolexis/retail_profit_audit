...
const detailLabel = (key: string, level: DetailLevel) => level === "months" ? formatBusinessMonth(key.slice(0, 7), { month: "short", year: "numeric" }) : level === "weeks" ? `Нед. ${formatBusinessCalendarDate(key, { day: "2-digit", month: "short" })}` : formatBusinessCalendarDate(key, { day: "2-digit", month: "short" });
...
export default function Inventory() {
  ...
  const inventoryTableRows = trendData.filter(row => chartTableMatches(tableQuery, row.month, row[trend]));
  const inventoryTableSummaryLabel = tableQuery.trim() ? "Итого по выборке" : trend === "stockOpen" ? "На начало" : trend === "stockClose" ? "На конец" : "Итого";
  const inventoryTableSummary = trend === "stockOpen"
    ? Number(inventoryTableRows[0]?.[trend] ?? 0)
    : trend === "stockClose"
      ? Number(inventoryTableRows.at(-1)?.[trend] ?? 0)
      : inventoryTableRows.reduce((sum, row) => sum + Number(row[trend] ?? 0), 0);
