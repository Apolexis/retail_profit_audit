  ...
  return <AuditShell kicker="25 / ИНВЕНТАРИЗАЦИИ" title="Инвентаризации">
    ...
    <section className="inventory-layout">
      <form className="packet-card inventory-create-card" onSubmit={openInventory}>
        <div className="card-title"><div><span>{active ? "ОТКРЫТЫЙ ПЕРЕСЧЕТ" : "НОВЫЙ ПЕРЕСЧЕТ"}</span><h3>{active ? `${active.storeName} · ${displayDate(active.businessDate)}` : "Открыть черновик"}</h3></div>{active ? <div className="inventory-draft-actions"><button type="button" className="subtle-button" onClick={startNewInventory}>Новый пересчет</button>{active.status === "draft" && <ConfirmDangerDialog trigger={<button type="button" className="subtle-button subtle-danger" disabled={deleteDraft.isPending}><Trash2 size={14}/>{deleteDraft.isPending ? "Удаляем…" : "Удалить черновик"}</button>} title="Удалить черновик?" description="Черновик и его строки будут удалены. Учетные остатки не изменятся, потому что ревизия еще не закрыта. Действие останется в общем журнале." confirmLabel="Удалить черновик" disabled={deleteDraft.isPending} onConfirm={() => deleteDraft.mutate({ inventoryId: active.id })}/>}</div> : <ClipboardCheck size={20}/>}</div>
