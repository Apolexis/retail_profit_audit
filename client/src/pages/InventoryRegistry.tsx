...
export default function InventoryRegistry() {
  ...
  const sortedLines = useMemo(() => {
    ...
    return [...active.lines].sort((left, right) => sortLinesByCategory
  ...
  return <AuditShell kicker="25 / ИНВЕНТАРИЗАЦИИ" title="Инвентаризации">
    ...
    <section className="inventory-layout">
      ...
      {active && <section className="packet-card inventory-draft-card">
        ...
        {active.status === "draft" && <form className="inventory-line-create" onSubmit={addLine}><label>Поиск товара<div className="inventory-search"><input value={productSearch} onChange={event => { setProductSearch(event.target.value); setProductId(""); }} placeholder="Название или внутренний код"/><Search size={15}/></div></label><label>Товар<ThemedSelect value={productId} onChange={event => setProductId(event.target.value)} disabled={!visibleProducts.length}><option value="">{products.isLoading ? "Загружаем справочник…" : visibleProducts.length ? "Выберите товар" : "Нет доступных товаров"}</option>{visibleProducts.map(product => <option key={product.id} value={product.id}>{product.canonicalName}{product.variant ? ` · ${product.variant}` : ""} · {catalogUnitLabel[product.baseUnit]}</option>)}</ThemedSelect></label>{!isSeller && <div className="inventory-accounting-readout"><span>Учетный остаток</span><strong>{selectedProduct?.accountingQuantity === null || !selectedProduct ? "Нет данных" : `${quantityText(selectedProduct.accountingQuantity)} ${catalogUnitLabel[selectedProduct.baseUnit]}`}</strong></div>}<label className="inventory-quantity-field"><span>Фактический остаток</span><div className="inventory-quantity-input"><input data-decimal-input type="text" inputMode="decimal" pattern="[0-9]*[.]?[0-9]*" value={quantity} onChange={event => setQuantity(normalizeDecimalInputText(event.target.value).replace(/[^0-9.]/g, ""))} placeholder="0" disabled={!selectedProduct}/><small>{selectedProduct ? catalogUnitLabel[selectedProduct.baseUnit] : "Ед."}</small></div></label><button className="packet-link" disabled={!selectedProduct || !quantity || upsertLine.isPending}><Plus size={16}/>{upsertLine.isPending ? "Добавляем…" : "Добавить в ревизию"}</button><a className="subtle-button inventory-add-product-link" href={`/catalog-control/new?return=${encodeURIComponent(`/inventory-control?store=${active.storeId}`)}`}><PackagePlus size={14}/>Нет товара</a></form>}
    ...
    <section className="packet-card inventory-history"><div className="card-title"><div><span>{isSeller ? "МОИ ПЕРЕСЧЕТЫ" : "ИСТОРИЯ ИНВЕНТАРИЗАЦИЙ"}</span><h3>{isSeller ? "До пяти последних пересчетов точки" : "Последние пересчеты выбранной точки"}</h3></div></div>{history.isLoading ? <p className="packet-note">Загружаем историю пересчетов…</p> : (history.data as InventoryListItem[] | undefined)?.length ? <div className="inventory-history-list">{(history.data as InventoryListItem[]).map(item => <article className={item.id === activeInventoryId ? "selected" : ""} key={item.id}><button type="button" className="inventory-history-select" onClick={() => { setActiveInventoryId(item.id); setStoreId(String(item.storeId)); setBusinessDate(item.businessDate); setNote(item.note ?? ""); }}><span>{displayDate(item.businessDate)}</span><strong>{item.storeName}</strong><small>{item.status === "draft" ? "Черновик · доступно изменение" : "Закрыта · корректировки созданы"}</small><b>{item.status === "draft" ? "Черновик" : "Закрыта"}</b></button>{item.status === "draft" && <ConfirmDangerDialog trigger={<button type="button" className="subtle-button subtle-danger" disabled={deleteDraft.isPending}><Trash2 size={14}/>{deleteDraft.isPending ? "Удаляем…" : "Удалить"}</button>} title="Удалить черновик?" description="Черновик и его строки будут удалены. Учетные остатки не изменятся, потому что ревизия еще не закрыта. Действие останется в общем журнале." confirmLabel="Удалить черновик" disabled={deleteDraft.isPending} onConfirm={() => deleteDraft.mutate({ inventoryId: item.id })}/>}</article>)}</div> : <div className="empty-state compact"><ClipboardCheck size={25}/><h2>Пересчетов пока нет</h2><p>После открытия первого черновика здесь появится история вашей точки.</p></div>}</section>
  </AuditShell>;
}
