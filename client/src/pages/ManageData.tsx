...
import { moscowBusinessDate } from "@/lib/utils";

const choices = ["revenue", "sales_smoked", "sales_frozen", "cash_revenue", "cashless_revenue", "receipts_total", "gross_profit", "net_profit", "purchases", "purchase_smoked", "purchase_frozen", "markup_smoked", "markup_frozen", "markup_total", "writeoff_smoked", "writeoff_frozen", "movement", "discount", "revaluation", "household", "delivery", "cleaning", "bonus", "seniority", "supplement", "driver_cash", "utilities_cash", "operating_costs", "cashless_operating_costs", "personal_income_tax_22", "driver_cashless", "utilities_cashless", "rent", "bank_fee", "gross_profit_tax", "salary_cashless", "payroll_tax", "vacation_cashless", "vacation_tax", "salary_cash", "vacation_cash", "cash_operating_costs", "stock_open", "stock_close"] as const;
const labels: Record<string, string> = { revenue: "Продажи общие", cash_revenue: "Выручка нал", cashless_revenue: "Выручка б/нал", receipts_total: "Выручка общая", gross_profit: "Валовая прибыль", net_profit: "Чистая прибыль", purchases: "Закупки", purchase_smoked: "Закупка Коп.", purchase_frozen: "Закупка Мор.", sales_smoked: "Продажи Коп.", sales_frozen: "Продажи Мор.", markup_smoked: "% накрутки копченой", markup_frozen: "% накрутки мороженой", markup_total: "% накрутки общий", writeoff_smoked: "Списания К.", writeoff_frozen: "Списания М.", movement: "Перемещения", discount: "Уценка", revaluation: "Переоценка", household: "Хоз. нужды нал", delivery: "Доставка нал", cleaning: "Уборка нал", bonus: "Премия нал", seniority: "Выслуга нал", supplement: "Доплата нал", driver_cash: "Водитель нал", utilities_cash: "Коммунальные нал", operating_costs: "Расходы нал", cashless_operating_costs: "Расходы б/нал", cash_operating_costs: "Общие траты нал", driver_cashless: "Водитель б/нал", utilities_cashless: "Коммуналка б/нал", rent: "Аренда б/нал", bank_fee: "% банку", gross_profit_tax: "Налоги", salary_cashless: "Зарплата б/нал", payroll_tax: "Налоги зарплатные", vacation_cashless: "Отпускные б/нал", vacation_tax: "Налоги на отпускные", salary_cash: "Зарплата нал", vacation_cash: "Отпускные нал", personal_income_tax_22: "НДФЛ 22%", stock_open: "Остаток на начало", stock_close: "Остаток на конец" };
const calculatedMetricCodes = new Set(["purchases", "stock_close", "receipts_total", "revenue", "markup_smoked", "markup_frozen", "markup_total", "gross_profit", "net_profit", "cash_operating_costs", "bank_fee", "gross_profit_tax", "personal_income_tax_22"]);
const isCalculatedMetric = (metricCode: string) => calculatedMetricCodes.has(metricCode);
...
export default function ManageData() {
  ...
  return <AuditShell kicker="11 / УПРАВЛЕНИЕ БАЗОЙ" title="Магазины и факты">
    ...
    {!stores.data?.length ? <section className="empty-state"><h2>База фактов пока пуста</h2><p>Подтвердите первый импорт книги Excel — здесь появятся магазины, даты и показатели.</p><Link href="/import" className="packet-link">Перейти к импорту</Link></section> : <section className="manage-grid">
      <article className="packet-card"><div className="card-title manage-store-title"><div><span>МАГАЗИНЫ</span><h3>Видимость в анализе</h3></div><label className="store-list-sort">Сортировка<ThemedSelect value={storeListSort} onChange={event => setStoreListSort(event.target.value as typeof storeListSort)}><option value="name">По названию А—Я</option><option value="visible">Сначала в анализе</option><option value="hidden">Сначала скрытые</option></ThemedSelect></label></div><div className="store-manage-list">{sortedStoreList.map(item => <div key={item.id}><span><b>{item.name}</b><small>{item.isHidden ? "скрыт из срезов" : "включен в срезы"}</small></span><button type="button" className="store-action" onClick={() => visibility.mutate({ id: item.id, isHidden: !item.isHidden })} title={item.isHidden ? "Показать" : "Скрыть"}>{item.isHidden ? <EyeOff size={17}/> : <Eye size={17}/>}</button><button type="button" onClick={() => { setStoreId(item.id); initializedStoreId.current = null; resetEditor(); }} className={storeId === item.id ? "store-action active" : "store-action"} title="Редактировать факты"><PencilLine size={16}/></button></div>)}</div></article>
      <article id="fact-editor" className="packet-card manage-editor"><div className="card-title"><div><span>{editingCode ? "РЕДАКТИРОВАНИЕ ПОКАЗАТЕЛЯ" : "НОВЫЙ ПОКАЗАТЕЛЬ"}</span><h3>{selected?.name ?? "Выберите магазин"}</h3></div><button type="button" className="subtle-action store-rename-toggle" onClick={() => setRenamingStore(value => !value)} disabled={!selected}>{renamingStore ? <X size={14}/> : <PencilLine size={14}/>}{renamingStore ? "Отмена" : "Переименовать магазин"}</button></div>
        ...
        {editingCode ? <><div className={isCalculatedMetric(editingCode) ? "editing-metric manage-editing-metric calculated-metric-notice" : "editing-metric manage-editing-metric"}><span>Редактируемая строка{isCalculatedMetric(editingCode) && <b className="calculated-metric-badge">Рассчитано автоматически</b>}</span><strong>{editingLabel}</strong><small>{isCalculatedMetric(editingCode) ? "Показатель рассчитан в исходной книге. Меняйте его вручную только после проверки первичных строк и формулы." : "Сумму можно сохранить отдельно. Тип меняется только после явного подтверждения ниже."}</small></div><div className="edit-controls fact-editor"><label>Сумма, ₽<input type="text" inputMode="decimal" pattern="[0-9]*[.]?[0-9]*" value={amount} onChange={event => setAmount(event.target.value)}/></label><button type="button" className="subtle-action metric-rename-action" onClick={() => { setNextMetricCode(editingCode); setChangingMetricType(value => !value); }}><Tag size={14}/>{changingMetricType ? "Отменить смену типа" : "Изменить тип"}</button></div>{changingMetricType && <div className="metric-type-panel"><label>Новый тип показателя<ThemedSelect value={nextMetricCode} onChange={event => setNextMetricCode(event.target.value)}>{choices.map(key => <option key={key} value={key}>{labels[key]}{isCalculatedMetric(key) ? " · авторасчет" : ""}</option>)}</ThemedSelect><small>Сумма и статус видимости сохранятся.</small></label><div className="metric-type-actions"><button type="button" className="packet-link compact metric-rename-action" disabled={renameMetric.isPending || nextMetricCode === editingCode} onClick={submitMetricRename}><Tag size={14}/>{renameMetric.isPending ? "Сохраняем…" : "Подтвердить тип"}</button></div></div>}</> : <div className="edit-controls fact-editor"><label>Новый показатель<ThemedSelect value={code} onChange={event => { setCode(event.target.value); setMessage(""); }}>{choices.map(key => <option key={key} value={key}>{labels[key]}{isCalculatedMetric(key) ? " · авторасчет" : ""}</option>)}</ThemedSelect><small>{isCalculatedMetric(code) ? "Рассчитано автоматически: меняйте только после проверки первичных строк и формулы." : "Добавляется новая строка на выбранную дату."}</small></label><label>Сумма, ₽<input type="text" inputMode="decimal" pattern="[0-9]*[.]?[0-9]*" value={amount} onChange={event => setAmount(event.target.value)}/><small>Для изменения уже добавленного показателя используйте кнопку в таблице.</small></label></div>}
