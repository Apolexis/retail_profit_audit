...
function FactDatePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  useEffect(() => { if (open) setDisplayMonth(date ?? new Date()); }, [open, value]);
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><button type="button" className="fact-date-trigger"><CalendarDays size={15}/>{date ? format(date, "d MMMM yyyy", { locale: ru }) : "Выбрать дату"}</button></PopoverTrigger><PopoverContent className="fact-date-popover" align="start" sideOffset={8}><div className="fact-date-popover-head"><span>ДАТА ФАКТА</span><b>{date ? format(date, "d MMM yyyy", { locale: ru }) : "Выберите дату"}</b></div><Calendar mode="single" locale={ru} selected={date} month={displayMonth} onMonthChange={setDisplayMonth} showOutsideDays={false} onSelect={next => { if (next) { onChange(format(next, "yyyy-MM-dd")); setOpen(false); } }}/><p>Один тап выбирает дату и закрывает календарь.</p></PopoverContent></Popover>;
}
