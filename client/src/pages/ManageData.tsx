...
import { Eye, EyeOff, PencilLine, Plus, RefreshCw, ShieldAlert, Tag, Trash2, X } from "lucide-react";
...
export default function ManageData() {
  ...
  const backfillPaymentRevenue = trpc.audit.backfillPaymentRevenue.useMutation({ onSuccess: result => { setMessage(result.filledMetrics ? `Добавлено ${result.filledMetrics} отсутствующих показателей выручки нал / б/нал из ${result.scannedBooks} исходных книг. Существующие факты не изменялись.` : "Выручка нал / б/нал уже заполнена во всех доступных фактах. Существующие значения не изменялись."); refresh(); }, onError: error => setMessage(error.message) });
  ...
  return <AuditShell kicker="11 / УПРАВЛЕНИЕ БАЗОЙ" title="Магазины и факты">
    <section className="page-lede"><div><h2>Точная запись: магазин, дата, показатель, сумма</h2><p>Импортные месяцы хранятся датой первого числа. Выберите дату календарем: если в выбранном месяце уже есть импорт, показатели появятся ниже. Любой факт можно скрыть из аналитики без удаления из базы.</p></div><div className="page-lede-actions"><button type="button" className="packet-link compact" onClick={() => backfillPaymentRevenue.mutate()} disabled={backfillPaymentRevenue.isPending}><RefreshCw size={15}/>{backfillPaymentRevenue.isPending ? "Дополняем…" : "Дополнить выручку нал / б/нал"}</button><small>Читает исходные книги и добавляет только отсутствующие строки; существующие факты не заменяет.</small></div></section>
