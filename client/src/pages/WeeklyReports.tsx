...
export default function WeeklyReports(){
  ...
  const exportPdf=async()=>{
    ...
    try{
      ...
      const theme=document.documentElement.dataset.auditTheme;
      const canvas=await html2canvas(reportPdfRef.current,{backgroundColor:theme==="light"?"#f8fbff":"#0c0b12",scale:Math.min(2,window.devicePixelRatio||1),useCORS:true,logging:false});
  ...
  return <AuditShell kicker="16 / ОТЧЕТЫ" title="Регулярный отчет руководителя">
    ...
    {reports.isLoading?<section className="packet-card"><p className="packet-note">Загружаем сохраненные отчеты…</p></section>:!summary?<section className="empty-state"><FileText size={30}/><h2>Отчетов пока нет</h2><p>Сформируйте первую сводку вручную. Далее отчет будет появляться только если расписание включено.</p></section>:<>
      ...
      <div className="report-export-actions"><button type="button" className="packet-link compact report-export-button" disabled={isExporting} onClick={()=>void exportPdf()} aria-label="Выгрузить выбранный отчет в PDF"><Download size={15}/>{isExporting?"Готовим PDF…":"Выгрузить PDF"}</button></div>
    </>}
  </AuditShell>;
}
