...
export type ReportTimelinePoint={date:string;revenue:number;cashRevenue:number;cashlessRevenue:number;netProfit:number;stores:number};
...
const savePdfForDevice=async(pdf:BrowserPdf,fileName:string)=>{const blob=pdf.output("blob");const file=new File([blob],fileName,{type:"application/pdf"});const shareNavigator=navigator as Navigator&{canShare?:(data:{files:File[]})=>boolean;share?:(data:{files:File[];title:string;text:string})=>Promise<void>};if(shareNavigator.canShare?.({files:[file]})&&shareNavigator.share){try{await shareNavigator.share({files:[file],title:"Аналитика «Рыбный»",text:"Выбранный отчет руководителя"});return "shared" as const;}catch(error){if(error instanceof DOMException&&error.name==="AbortError")return "cancelled" as const;throw error;}}const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=fileName;link.style.display="none";document.body.append(link);link.click();link.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1000);return "downloaded" as const;};
...
export const reportTimeline=(periods:readonly ReportTimelinePeriod[]):ReportTimelinePoint[]=>{
  const byDate=new Map<string,{revenue:number;cashRevenue:number;cashlessRevenue:number;netProfit:number;stores:Set<string>}>();
  periods.filter(period=>!period.isHidden).forEach(period=>{
    const current=byDate.get(period.entryDate)??{revenue:0,cashRevenue:0,cashlessRevenue:0,netProfit:0,stores:new Set<string>()};
  ...
  return Array.from(byDate.entries()).sort(([left],[right])=>left.localeCompare(right)).map(([date,point])=>({date,revenue:point.revenue,cashRevenue:point.cashRevenue,cashlessRevenue:point.cashlessRevenue,netProfit:point.netProfit,stores:point.stores.size}));
};
...
export default function WeeklyReports(){
  ...
  const exportPdf=async()=>{
    ...
    try{
      ...
      const captureScale=Math.min(1.35,Math.max(1,window.devicePixelRatio>1?1.2:1));
      const canvas=await html2canvas(reportPdfRef.current,{backgroundColor:pdfTheme==="light"?"#f8fbff":"#0c0b12",scale:captureScale,useCORS:true,logging:false});
