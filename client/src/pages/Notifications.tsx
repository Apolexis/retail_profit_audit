import { ArrowUpRight, BellRing, CheckCheck, CheckCircle2, CircleAlert, Info, Megaphone, Send, TriangleAlert } from "lucide-react";
...
import { russianCountNoun } from "@/lib/russianPlural";
...
type EvotorPushSettings = { applicationId: string | null; updatedAt: Date | null };
type EvotorPushRecipient = { storeId: number; storeName: string; terminalCount: number };
const terminalCountLabel = (count: number) => `${count} ${russianCountNoun(count, ["касса", "кассы", "касс"])}`;
const storeCountLabel = (count: number) => `${count} ${russianCountNoun(count, ["магазине", "магазинах", "магазинах"])}`;
...
function ThresholdRuleCard({ rule, draft, onDraftChange, onSave, isSaving }: { rule: ThresholdRule; draft: ThresholdDraft; onDraftChange: (next: ThresholdDraft) => void; onSave: () => void; isSaving: boolean }) {
  ...
  return <form className={`threshold-rule ${draft.isEnabled ? "" : "is-disabled"}`} onSubmit={event => { event.preventDefault(); onSave(); }}>
    ...
    <label className="threshold-enabled threshold-switch"><input type="checkbox" checked={draft.isEnabled} onChange={event => onDraftChange({ ...draft, isEnabled: event.target.checked })}/><span className="threshold-switch-track" aria-hidden="true"/><span>{draft.isEnabled ? "Включен" : "Отключен"}</span></label>
...
export default function Notifications() {
  ...
  const notificationSummary = trpc.localAuth.notificationSummary.useQuery(undefined, { retry: false, refetchInterval: 60_000 });
  const notifications = trpc.localAuth.notifications.useInfiniteQuery({ limit: 50 }, { getNextPageParam: page => page.nextCursor ?? undefined, retry: false, refetchInterval: 60_000 });
  ...
  const evotorPushSettings = trpc.inventoryRegistry.evotorPushSettings.useQuery(undefined, { enabled: me.data?.role === "admin", retry: false });
  const evotorPushRecipients = trpc.inventoryRegistry.evotorPushRecipients.useQuery(undefined, { enabled: me.data?.role === "admin", retry: false });
  ...
  const [pushApplicationId, setPushApplicationId] = useState("");
  ...
  useEffect(() => {
    const settings = evotorPushSettings.data as EvotorPushSettings | undefined;
    if (settings?.applicationId && !pushApplicationId) setPushApplicationId(settings.applicationId);
  }, [evotorPushSettings.data, pushApplicationId]);
  useEffect(() => {
  ...
  const savePushApplication = trpc.inventoryRegistry.setEvotorPushApplication.useMutation({ onSuccess: async () => { await evotorPushSettings.refetch(); toast.success("application_id push Эвотор сохранён"); }, onError: error => toast.error("application_id push не сохранён", { description: error.message }) });
  const sendEvotorPush = trpc.inventoryRegistry.sendEvotorPush.useMutation({ onSuccess: result => { setPushMessage(""); toast.success(`Push передан на ${terminalCountLabel(result.recipientCount)} в ${storeCountLabel(result.storeCount)}.`, { description: result.storeNames.join(", ") }); }, onError: error => toast.error("Push на кассы не отправлен", { description: error.message }) });
  ...
  const pushSettings = evotorPushSettings.data as EvotorPushSettings | undefined;
  const pushRecipients = (evotorPushRecipients.data ?? []) as EvotorPushRecipient[];
  ...
  const openSource = (item: { id: number; entityType: string | null; entityId: string | null }) => {
    ...
    if (item.entityType === "operational_signal") { const returnId=item.entityId?.match(/^evotor_return:(\d+)$/)?.[1]; setLocation(returnId ? `/evotor-sales/receipts?receipt=${returnId}` : item.entityId?.startsWith("request_missing:") ? "/requests" : item.entityId?.startsWith("overstock_cover:") || item.entityId?.startsWith("evotor_stock_freshness:") ? "/stock-control" : item.entityId?.startsWith("evotor_receipt_freshness:") ? "/evotor-sales/receipts" : item.entityId?.startsWith("expiry_") ? "/notifications" : "/revenue"); return; }
  ...
  const sourceLabel = (item: { entityType: string | null; entityId?: string | null }) => item.entityType === "import" ? "Открыть импорт" : item.entityType === "price" ? "Открыть прайс‑контроль" : item.entityType === "alert_feed" ? "Открыть детали" : item.entityType === "metric" ? "Открыть факт" : item.entityType === "threshold" ? "Открыть источник" : item.entityType === "operational_signal" && item.entityId?.startsWith("evotor_return:") ? "Открыть возврат" : item.entityType === "operational_signal" && (item.entityId?.startsWith("overstock_cover:") || item.entityId?.startsWith("evotor_stock_freshness:")) ? "Открыть остатки" : item.entityType === "operational_signal" && item.entityId?.startsWith("evotor_receipt_freshness:") ? "Открыть чеки Эвотор" : item.entityType === "operational_signal" && item.entityId?.startsWith("expiry_") ? null : item.entityType === "operational_signal" ? "Открыть реестр" : item.entityType === "store" ? "Открыть магазин" : item.entityType === "weekly_report" ? "Открыть отчет" : null;

  return <AuditShell kicker="15 / УВЕДОМЛЕНИЯ" title="Сигналы и контроль">
    <section className="page-lede"><div><span>ЦЕНТР СОБЫТИЙ</span><h2>Критичные изменения под контролем</h2><p>Сигналы появляются при нарушении выбранных порогов, существенных ручных изменениях и замене периодов при импорте. Финансовые события и контроль покрытия запаса получают административные адресаты, операционные дедлайны — назначенный продавец, а срок партии 1С — руководитель и сам магазин.</p></div></section>
    ...
    {me.data?.role === "admin" && <section className="packet-card admin-broadcast"><div className="card-title"><div><span>СООБЩЕНИЕ ПОЛЬЗОВАТЕЛЯМ</span><h3>Рассылка в ленту сигналов</h3></div><Megaphone size={20}/></div><p className="packet-note">Сообщение попадет в ленту активных получателей. Push‑уведомление придет только на устройство с добровольно включенной браузерной подпиской.</p><form className="stack-form" onSubmit={event => { event.preventDefault(); if (broadcastText.trim() && (broadcastTarget !== "account" || broadcastAccountId)) broadcast.mutate({ message: broadcastText.trim(), audience: broadcastAudience }); }}><div className="broadcast-targeting"><div className="broadcast-target-row"><span>Получатели</span><div className="broadcast-target-buttons" role="group" aria-label="Кому отправить сообщение"><button type="button" className={broadcastTarget === "all" ? "active" : ""} aria-pressed={broadcastTarget === "all"} onClick={() => setBroadcastTarget("all")}>Всем</button><button type="button" className={broadcastTarget === "account" ? "active" : ""} aria-pressed={broadcastTarget === "account"} onClick={() => setBroadcastTarget("account")}>Пользователю</button><button type="button" className={broadcastTarget === "role" ? "active" : ""} aria-pressed={broadcastTarget === "role"} onClick={() => setBroadcastTarget("role")}>По роли</button></div></div>{broadcastTarget === "account" && <label>Учетная запись<ThemedSelect value={broadcastAccountId} onChange={event => setBroadcastAccountId(event.target.value)} required><option value="">Выберите пользователя</option>{activeAccounts.map(account => <option key={account.id} value={account.id}>{formatLocalAccountLogin(account.username)}</option>)}</ThemedSelect></label>}{broadcastTarget === "role" && <label>Роль<ThemedSelect value={broadcastRole} onChange={event => setBroadcastRole(event.target.value as AccountRole)}><option value="analyst">Аналитики</option><option value="seller">Продавцы</option><option value="manager">Руководители</option><option value="admin">Администраторы</option></ThemedSelect></label>}</div><label>Текст сообщения<textarea value={broadcastText} onChange={event => setBroadcastText(event.target.value)} maxLength={360} placeholder="Например: завтра сверяем списания и остатки до 11:00." required/></label><div className="admin-broadcast-actions"><small>{broadcastText.trim().length}/360</small><button className="packet-link compact" disabled={!broadcastText.trim() || broadcast.isPending || (broadcastTarget === "account" && !broadcastAccountId)}><Megaphone size={15}/>{broadcast.isPending ? "Отправляем…" : "Отправить"}</button></div></form></section>}
    {me.data?.role === "admin" && <section className="packet-card admin-broadcast evotor-push-control"><div className="card-title"><div><span>ЭВОТОР · PUSH</span><h3>Сообщение в приложение на кассах</h3></div><Send size={20}/></div><p className="packet-note">При выборе магазина сообщение направляется на все обнаруженные кассы точки. Эвотор доставляет payload только в установленное приложение; это не системное уведомление кассы.</p><form className="stack-form" onSubmit={event => { event.preventDefault(); if (pushApplicationId.trim()) savePushApplication.mutate({ applicationId: pushApplicationId.trim() }); }}><label>Application ID приложения Эвотор<input value={pushApplicationId} onChange={event => setPushApplicationId(event.target.value)} placeholder="Из карточки установленного приложения Эвотор" autoComplete="off"/></label><div className="admin-broadcast-actions"><small>{pushSettings?.applicationId ? "ID сохранён; исходящие сообщения доступны ниже." : "Сначала сохраните ID установленного приложения."}</small><button className="packet-link compact" disabled={!pushApplicationId.trim() || savePushApplication.isPending}><BellRing size={15}/>{savePushApplication.isPending ? "Сохраняем…" : "Сохранить application ID"}</button></div></form><div className="stack-form evotor-push-send"><label>Получатели<ThemedSelect value={pushScope} onChange={event => setPushScope(event.target.value)}><option value="all">Все магазины ({terminalCountLabel(pushRecipients.reduce((total, recipient) => total + recipient.terminalCount, 0))})</option>{pushRecipients.map(recipient => <option value={String(recipient.storeId)} key={recipient.storeId}>{recipient.storeName} · все кассы ({terminalCountLabel(recipient.terminalCount)})</option>)}</ThemedSelect></label><label>Сообщение<textarea value={pushMessage} onChange={event => setPushMessage(event.target.value)} maxLength={1700} placeholder="Текст для приложения на кассе" rows={3}/></label><div className="admin-broadcast-actions"><small>{pushMessage.trim().length}/1700 · {pushRecipients.length} {russianCountNoun(pushRecipients.length, ["магазин", "магазина", "магазинов"])}</small><button type="button" className="packet-link compact" disabled={!pushMessage.trim() || !pushSettings?.applicationId || sendEvotorPush.isPending || !pushRecipients.length} onClick={() => sendEvotorPush.mutate({ message: pushMessage.trim(), storeIds: pushScope === "all" ? undefined : [Number(pushScope)] })}><Send size={15}/>{sendEvotorPush.isPending ? "Отправляем…" : pushScope === "all" ? "Отправить всем кассам" : "Отправить во все кассы магазина"}</button></div></div></section>}
