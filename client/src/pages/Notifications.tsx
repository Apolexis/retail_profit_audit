...
export default function Notifications() {
  ...
  return <AuditShell kicker="15 / УВЕДОМЛЕНИЯ" title="Сигналы и контроль">
    ...
    {me.data?.role === "admin" && <section className="packet-card alert-thresholds"><div className="card-title"><div><span>НАСТРОЙКИ ПОРОГОВ</span><h3>Когда отправлять сигнал</h3></div></div><p className="packet-note">Порог применяется при ручном изменении факта и после импорта. Отключенное правило не формирует системные и телефонные уведомления.</p>
      {cashControlRules.length > 0 && <section className="threshold-control-group"><div className="threshold-control-copy"><span>НАЛИЧНЫЕ РАСХОДЫ И НДФЛ 22%</span><strong>Связанный ежедневный контроль</strong><p>Операционные траты наличными и НДФЛ настраиваются рядом: это разные показатели, но одна зона проверки выплат.</p></div><div className="threshold-grid threshold-grid-cash">{cashControlRules.map(rule => <ThresholdRuleCard key={rule.ruleKey} rule={rule} draft={draftFor(rule)} onDraftChange={next => setDraft(rule.ruleKey, next)} onSave={() => saveRule(rule)} isSaving={saveThreshold.isPending}/>)}</div></section>}
