...
const entryFor = (amounts: AmountDraft, comments: CommentDraft) => ({ ...Object.fromEntries(fields.map(field => [field.key, parseAmount(amounts[field.key])])) as unknown as Record<AmountField, number>, expenseComments: comments });
const toMoscowDate = () => {
...
export default function RevenueRegistry() {
  ...
  const adminFilter = useMemo(() => ({ from: adminFrom || undefined, to: adminTo || undefined, storeId: adminStoreId ? Number(adminStoreId) : undefined }), [adminFrom, adminTo, adminStoreId]);
  ...
  const submit = (event: React.FormEvent) => {
    ...
    const entry = { ...entryFor(amounts, comments), expenseComments: comments };
    if (editing) {
