...
export default function RevenueRegistry() {
  ...
  return <AuditShell kicker="23 / ВЫРУЧКА" title="Операционный реестр «Выручка»">
    ...
    <section className="revenue-layout"><details className="packet-card revenue-entry-card" ref={entryCardRef}>
    ...
    {isAdmin && <section className="packet-card revenue-admin-register">
      ...
      {adminRecords.isLoading ? <p className="packet-note">Загружаем операционный реестр…</p> : currentRecords.length ? <>
        <p className="revenue-print-date">{displayDate(registryDate)}</p>
        <div className="data-table-wrap revenue-register-table-wrap" onPointerDown={beginRegisterDrag} onPointerMove={moveRegisterDrag} onPointerUp={endRegisterDrag} onPointerCancel={endRegisterDrag}>
          <table className="data-table revenue-register-table"><thead><tr><th>Магазин</th><th className="numeric-column">Нал</th><th className="numeric-column">Б/Нал</th>{visibleExpenseFields.map(field => <th className="numeric-column" key={field.key}>{field.label}</th>)}<th className="numeric-column">Итого</th><th className="revenue-not-for-print">Версия</th><th className="revenue-not-for-print">Действия</th></tr></thead><tbody>{currentRecords.map(record => <tr key={record.id}><td><strong>{record.storeName}</strong><small className="revenue-screen-only">{record.createdByName}</small></td><td className="numeric-column">{formatAmount(record.cash)}</td><td className="numeric-column">{formatAmount(record.cashless)}</td>{visibleExpenseFields.map(field => <td className="numeric-column" key={field.key}>{formatAmount(record[field.key])}</td>)}<td className="numeric-column"><strong>{formatAmount(record.total)}</strong></td><td className="revenue-not-for-print">v{record.currentVersion}</td><td className="revenue-not-for-print"><div className="revenue-row-actions"><button type="button" className="subtle-button" onClick={() => fillEdit(record)}><PencilLine size={14}/>Изменить</button><button type="button" className="subtle-button subtle-danger" onClick={() => { setVoidCandidate(record); setVoidReason(""); }}><Trash2 size={14}/>Удалить</button></div></td></tr>)}</tbody><tfoot><tr className="table-total"><th scope="row">Итого</th><td className="numeric-column">{formatAmount(currentRecords.reduce((sum, record) => sum + record.cash, 0))}</td><td className="numeric-column">{formatAmount(currentRecords.reduce((sum, record) => sum + record.cashless, 0))}</td>{visibleExpenseFields.map(field => <td className="numeric-column" key={field.key}>{formatAmount(currentRecords.reduce((sum, record) => sum + record[field.key], 0))}</td>)}<td className="numeric-column">{formatAmount(currentRecords.reduce((sum, record) => sum + record.total, 0))}</td><td className="revenue-not-for-print" colSpan={2}/></tr></tfoot></table>
        </div>
        {printComments.length > 0 && <section className="revenue-print-summary" aria-label="Комментарии к расходам"><div className="revenue-print-comments">{printComments.map(({ record, field }) => <p key={`${record.id}:${field.key}`}><strong>{record.storeName} · {field.label} · {formatAmount(record[field.key])}</strong><span>{record.expenseComments[field.key as ExpenseField]}</span></p>)}</div></section>}
