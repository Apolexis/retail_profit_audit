      ...
      {adminRecords.isLoading ? <p className="packet-note">Загружаем операционный реестр…</p> : currentRecords.length ? <>
        <div className="data-table-wrap revenue-register-table-wrap" onPointerDown={beginRegisterDrag} onPointerMove={moveRegisterDrag} onPointerUp={endRegisterDrag} onPointerCancel={endRegisterDrag}>
          <table className="data-table revenue-register-table">
            ...
            <tbody>{currentRecords.map(record => <tr key={record.id}><td><strong>{record.storeName}</strong><small className="revenue-screen-only">{record.createdByName}</small></td><td className="numeric-column">{formatAmount(record.cash)}</td><td className="numeric-column">{formatAmount(record.cashless)}</td>{visibleExpenseFields.map(field => <td className="numeric-column" key={field.key}>{formatAmount(record[field.key])}</td>)}<td className="numeric-column"><strong>{formatAmount(record.total)}</strong></td>{hasExpenseComments && <td className="revenue-comment-cell">{formatRecordComments(record) || "—"}</td>}<td>v{record.currentVersion}</td><td><div className="revenue-row-actions"><button type="button" className="subtle-button" onClick={() => fillEdit(record)}><PencilLine size={14}/>Изменить</button><button type="button" className="subtle-button subtle-danger" onClick={() => { setVoidCandidate(record); setVoidReason(""); }}><Trash2 size={14}/>Удалить</button></div></td></tr>)}</tbody>
