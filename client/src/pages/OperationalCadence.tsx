...
export default function OperationalCadence() {
  ...
  const evotorSyncStatus = trpc.inventoryRegistry.evotorSyncStatus.useQuery(undefined, { retry: false, enabled: !demoMode && needsEvotorFacts });
  ...
  const syncStatus = demoMode ? null : evotorSyncStatus.data;
  const currentDayCycleNote = !needsEvotorFacts
    ? "Выберите факт Эвотор: тогда загрузятся только агрегаты чеков и состояние обхода."
    : demoMode
      ? "В демо-режиме реальные чеки и состояние синхронизации не читаются."
      : evotorSyncStatus.isLoading
        ? "Проверяем состояние отдельного current-day обхода…"
        : !syncStatus?.mappedStores
          ? "Нет видимых закрепленных точек для read-only загрузки Эвотор."
          : !syncStatus.documentJob.active
            ? "Минутная задача документов сейчас не активна."
            : `Сегодня: начато ${syncStatus.currentDay.startedStores} из ${syncStatus.mappedStores}, завершено ${syncStatus.currentDay.completedStores}. Один callback читает одну точку и одну API-страницу; первый проход занимает до ${syncStatus.mappedStores} минут. Следующие страницы текущего дня завершаются до перехода к архиву с ${syncStatus.retentionStart}.`;
  ...
  const expenseBreakdownTotal = expenseBreakdownLedger.reduce((sum, row) => sum + row.amount, 0);
  ...
  const chartTableTotals = chartLines.map(line => chartData.reduce((sum, point) => sum + Number((point as Record<string, string | number | undefined>)[line.key] ?? 0), 0));
  ...
  return <AuditShell kicker="08 / ОПЕРАЦИОННЫЙ РИТМ" title="Неделя, день и месяц: расчетный срез">
    {facts.loading ? <FactsLoader/> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30}/><h2>Нет фактов для расчетного среза</h2><p>Подтвердите импорт Excel, чтобы увидеть месячную, недельную и дневную динамику.</p></section> : <>
      ...
      <p className="packet-note cadence-evotor-sync-note"><Layers3 size={15}/> {currentDayCycleNote}</p>
