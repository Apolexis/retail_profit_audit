...
export default function OperationalCadence() {
  ...
  return <AuditShell kicker="08 / ОПЕРАЦИОННЫЙ РИТМ" title="Неделя и день: расчетный срез">
    {facts.loading ? <section className="empty-state"><p>Загружаем факты…</p></section> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30}/><h2>Нет фактов для расчетного среза</h2><p>Подтвердите импорт Excel, чтобы увидеть недельную и дневную динамику.</p></section> : <>
      <section className="page-lede"><div><h2>Операционный ритм по фактическим дням.</h2><p>Продажи, закупки, списания и остальные операционные показатели берутся из <b>первичных дневных строк</b> книги. Равномерно по календарному месяцу разносятся только согласованные ежемесячные статьи: аренда, зарплата, отпускные, часть операционных расходов и итоговая чистая прибыль. НДФЛ 22%, банк и налоги остаются на фактической дате. Остатки здесь не распределяются.</p></div><div className="page-controls"><label>Магазин<select value={all ? "__all__" : selectedStore} onChange={event => setSelectedStore(event.target.value)}><option value="__all__">Все магазины</option>{facts.storeNames.map(store => <option key={store} value={store}>{store}</option>)}</select></label></div></section>
      ...
      <section className="packet-split"><article className="packet-card"><div className="card-title"><div><span>КАК ЧИТАТЬ СРЕЗ</span><h3>Разделение источников</h3></div><Layers3 size={19}/></div><div className="detail-stat"><span>Продажи, закупки, списания, НДФЛ, банк и налоги</span><strong>фактический день</strong></div><div className="detail-stat"><span>Аренда, зарплата, отпускные и согласованные расходы</span><strong>равномерно по месяцу</strong></div><div className="detail-stat"><span>Итоговая чистая прибыль</span><strong>равномерно по месяцу</strong></div><div className="detail-stat"><span>Остаток на начало / конец</span><strong>не входит</strong></div><p className="packet-note">Для управления темпом продаж, прибылью, закупками и расходами используйте этот раздел. Для остатков и покрытия используйте отдельную страницу «Остатки».</p></article><article className="packet-card"><div className="card-title"><div><span>ПОСЛЕДНИЕ ИНТЕРВАЛЫ</span><h3>{metrics[metric].label}</h3></div><CalendarDays size={19}/></div><div className="inline-table"><div className="inline-row header"><span>{granularity === "week" ? "Неделя" : "Дата"}</span><span>Значение</span></div>{tableRows.map(point => <div className="inline-row" key={point.date}><span>{point.month}</span><strong>{formatK(point[metric] / 1000)}</strong></div>)}</div></article></section>
    </>}
  </AuditShell>;
}
