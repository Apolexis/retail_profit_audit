...
import { MetricLineChart, StoreSeriesModeToggle, formatK } from "@/components/AuditCharts";
...
export default function OperationalCadence() {
  ...
  const perStoreData = useMemo(() => {
    if (!showStoreSeries || !networkSelected || selectedMetrics.length !== 1) return null;
  ...
  }, [facts.rowsFor, facts.storeNames, granularity, networkSelected, range, selectedMetrics, showStoreSeries]);
  ...
  return <AuditShell kicker="08 / ОПЕРАЦИОННЫЙ РИТМ" title="Неделя, день и месяц: расчетный срез">
    {facts.loading ? <FactsLoader/> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30}/><h2>Нет фактов для расчетного среза</h2><p>Подтвердите импорт Excel, чтобы увидеть месячную, недельную и дневную динамику.</p></section> : <>
      ...
      {selectedMetrics.includes("expenses") && <section className="packet-card expense-breakdown-card"><div className="card-title"><div><span>СОСТАВ РАСХОДОВ · {scope}</span><h3>Какие исходные статьи формируют общую расходную нагрузку</h3><small>Показаны отдельные фактические статьи без строки «Расходы» и без двойного учета агрегатов.</small></div></div><MetricLineChart data={expenseBreakdown} lines={expenseBreakdownLines}/><p className="packet-note"><Calculator size={15}/> Используйте состав для проверки источника расхода: наличные, безналичные, ФОТ, налоги, аренда, банк и прочие статьи остаются отдельными рядами.</p></section>}
