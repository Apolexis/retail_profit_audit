import { Building2, Download, Link2, Save, Search, Tags, Warehouse } from "lucide-react";
...
export default function WarehouseControl() {
  ...
  return <AuditShell kicker="27 / СКЛАДЫ" title="Склады">
    ...
    <section className="packet-card warehouse-table-card"><div className="card-title"><div><span>СКЛАДЫ СЕТИ</span><h3>{rows.length} {rows.length === 1 ? "склад" : rows.length < 5 ? "склада" : "складов"}</h3></div><small>Виды цен управляются в «Номенклатуре»</small></div>
      ...
      {warehouses.isLoading || priceTypes.isLoading ? <p className="packet-note">Загружаем склады и виды цен…</p> : warehouses.isError ? <p className="packet-note">Склады недоступны: {warehouses.error.message}</p> : <><p className="warehouse-result-count">Показано: <strong>{visibleRows.length}</strong></p><div className="data-table-wrap warehouse-table-wrap"><table className="data-table warehouse-table"><thead><tr><th>Склад / магазин</th><th>Вид продажной цены</th><th>Эвотор</th><th aria-label="Действия"></th></tr></thead><tbody>{visibleRows.map(row => <tr key={row.storeId}><td><strong>{row.storeName}</strong>{row.isHidden && <small>Скрыт из рабочих выборок</small>}</td><td><ThemedSelect value={drafts[row.storeId] ?? ""} onChange={event => setDrafts(current => ({ ...current, [row.storeId]: event.target.value }))}><option value="" disabled>Выберите вид цены</option>{activePriceTypes.map(type => <option value={type.id} key={type.id}>{type.name}{type.isDefault ? " · основной" : ""}</option>)}</ThemedSelect></td><td><span className={row.hasEvotorMapping ? "warehouse-status is-linked" : "warehouse-status"}>{row.hasEvotorMapping ? <><Link2 size={14}/>Закреплен{row.evotorLinkedProductCount ? ` · ${row.evotorLinkedProductCount} поз.` : ""}</> : "Не задан"}</span></td><td><div className="warehouse-actions"><button type="button" className="subtle-button warehouse-save" disabled={!drafts[row.storeId] || Number(drafts[row.storeId]) === row.priceTypeId || assignPriceType.isPending} onClick={() => assignPriceType.mutate({ storeId: row.storeId, priceTypeId: Number(drafts[row.storeId]) })}><Save size={14}/>Назначить</button><button type="button" className="subtle-button warehouse-documents" disabled={!row.hasEvotorMapping || syncDocuments.isPending} onClick={() => syncDocuments.mutate({ storeId: row.storeId })}><Download size={14}/>Документы</button></div></td></tr>)}</tbody></table></div></>}
    </section>
