import { Building2, ChevronUp, Link2, Pencil, Search, Tags, Warehouse } from "lucide-react";
...
type WarehouseRow = { storeId: number; storeName: string; isHidden: boolean; priceTypeId: number | null; priceTypeName: string | null; hasEvotorMapping: boolean; evotorStoreName?: string | null; evotorLinkedProductCount: number };
...
export default function WarehouseControl() {
  ...
  return <AuditShell kicker="27 / СКЛАДЫ" title="Склады">
    ...
    <section className="packet-card warehouse-table-card"><div className="card-title"><div><span>СКЛАДЫ СЕТИ</span><h3>{visibleRows.length} {visibleRows.length === 1 ? "склад" : visibleRows.length < 5 ? "склада" : "складов"}</h3></div><small>Изменение открывает параметры только выбранного склада</small></div><div className="warehouse-filters"><label className="warehouse-search"><Search size={15}/><input value={warehouseSearch} onChange={event => setWarehouseSearch(event.target.value)} placeholder="Поиск склада / магазина" aria-label="Поиск склада или магазина"/></label><label>Показывать<ThemedSelect value={warehouseFilter} onChange={event => setWarehouseFilter(event.target.value as "all" | "mapped" | "not_mapped")}><option value="all">Все рабочие склады</option><option value="mapped">Только закрепленные Эвотор</option><option value="not_mapped">Без закрепления Эвотор</option></ThemedSelect></label></div>{warehouses.isLoading || priceTypes.isLoading || evotorChoices.isLoading ? <p className="packet-note">Загружаем настройки складов…</p> : warehouses.isError ? <p className="packet-note">Склады недоступны: {warehouses.error.message}</p> : <div className="data-table-wrap warehouse-table-wrap"><table className="data-table warehouse-table"><thead><tr><th>Склад / магазин</th><th>Вид цены</th><th>Эвотор</th><th aria-label="Настройка"/></tr></thead><tbody>{visibleRows.map(row => <Fragment key={row.storeId}><tr><td data-label="Склад / магазин"><strong>{row.storeName}</strong></td><td data-label="Вид цены">{row.priceTypeName ?? "Не назначен"}</td><td data-label="Эвотор"><span className={row.hasEvotorMapping ? "warehouse-status is-linked" : "warehouse-status"}>{row.hasEvotorMapping ? <><Link2 size={14}/>Закреплен{row.evotorLinkedProductCount ? ` · ${row.evotorLinkedProductCount} поз.` : ""}</> : "Не задан"}</span></td><td data-label="Действие"><button type="button" className="subtle-button warehouse-edit" onClick={() => setEditingStoreId(current => current === row.storeId ? null : row.storeId)}>{editingStoreId === row.storeId ? <ChevronUp size={14}/> : <Pencil size={14}/>} {editingStoreId === row.storeId ? "Свернуть" : "Изменить"}</button></td></tr>{editingStoreId === row.storeId && <tr className="warehouse-settings-row"><td colSpan={4}><div className="warehouse-settings-grid"><label>Вид продажной цены<ThemedSelect value={priceDrafts[row.storeId] ?? ""} onChange={event => { const value = event.target.value; setPriceDrafts(current => ({ ...current, [row.storeId]: value })); if (value && Number(value) !== row.priceTypeId) assignPriceType.mutate({ storeId: row.storeId, priceTypeId: Number(value) }); }}><option value="" disabled>Выберите вид цены</option>{activePriceTypes.map(type => <option value={type.id} key={type.id}>{type.name}{type.isDefault ? " · основной" : ""}</option>)}</ThemedSelect></label><label>Магазин Эвотор<ThemedSelect value={evotorDrafts[row.storeId] ?? ""} onChange={event => { const value = event.target.value; setEvotorDrafts(current => ({ ...current, [row.storeId]: value })); if (value) assignEvotorMapping.mutate({ storeId: row.storeId, evotorStoreId: value }); }}><option value="" disabled>Выберите точку из Эвотор</option>{choices.map((choice, index) => <option value={choice.id} key={choice.id}>{safeEvotorStoreLabel(choice.name, index)}</option>)}</ThemedSelect></label><p className="warehouse-settings-note">Настройки сохраняются при выборе. Синхронизация Эвотор выполняется автоматически и только на чтение.</p></div></td></tr>}</Fragment>)}</tbody></table></div>}</section>
