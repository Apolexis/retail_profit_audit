import { Building2, Link2, Save, Search, Tags, Warehouse } from "lucide-react";
...
export default function WarehouseControl() {
  ...
  const visibleRows = useMemo(() => {
    ...
    return query ? rows.filter(row => row.storeName.toLocaleLowerCase("ru-RU").includes(query)) : rows;
  }, [rows, warehouseSearch]);
  useEffect(() => {
    setDrafts(current => {
      const next = { ...current };
  ...
  return <AuditShell kicker="27 / СКЛАДЫ" title="Склады">
    ...
    <section className="packet-card warehouse-rule"><div><Link2 size={18}/><div><span>READ‑ONLY ЭВОТОР</span><strong>Связь точки закреплена, а не выбирается вручную</strong><p>Список показывает только факт закрепленного соответствия и количество связанных кассовых позиций. Запись в Эвотор, изменение кассы и показ адреса исключены.</p></div></div></section>

    <section className="packet-card warehouse-table-card"><div className="card-title"><div><span>СКЛАДЫ СЕТИ</span><h3>{rows.length} {rows.length === 1 ? "склад" : rows.length < 5 ? "склада" : "складов"}</h3></div><small>Виды цен управляются в «Номенклатуре»</small></div>
      ...
      {warehouses.isLoading || priceTypes.isLoading ? <p className="packet-note">Загружаем склады и виды цен…</p> : warehouses.isError ? <p className="packet-note">Склады недоступны: {warehouses.error.message}</p> : <><p className="warehouse-result-count">Показано: <strong>{visibleRows.length}</strong></p><div className="data-table-wrap warehouse-table-wrap"><table className="data-table warehouse-table"><thead><tr><th>Склад / магазин</th><th>Вид продажной цены</th><th>Эвотор</th><th aria-label="Действие"></th></tr></thead><tbody>{visibleRows.map(row => <tr key={row.storeId}><td><strong>{row.storeName}</strong>{row.isHidden && <small>Скрыт из рабочих выборок</small>}</td><td><ThemedSelect value={drafts[row.storeId] ?? ""} onChange={event => setDrafts(current => ({ ...current, [row.storeId]: event.target.value }))}><option value="" disabled>Выберите вид цены</option>{activePriceTypes.map(type => <option value={type.id} key={type.id}>{type.name}{type.isDefault ? " · основной" : ""}</option>)}</ThemedSelect></td><td><span className={row.hasEvotorMapping ? "warehouse-status is-linked" : "warehouse-status"}>{row.hasEvotorMapping ? <><Link2 size={14}/>Закреплен{row.evotorLinkedProductCount ? ` · ${row.evotorLinkedProductCount} поз.` : ""}</> : "Не задан"}</span></td><td><button type="button" className="subtle-button warehouse-save" disabled={!drafts[row.storeId] || Number(drafts[row.storeId]) === row.priceTypeId || assignPriceType.isPending} onClick={() => assignPriceType.mutate({ storeId: row.storeId, priceTypeId: Number(drafts[row.storeId]) })}><Save size={14}/>Назначить</button></td></tr>)}</tbody></table></div></>}
    </section>
