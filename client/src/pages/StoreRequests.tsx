  ...
  return <AuditShell kicker="30 / ЗАЯВКИ" title="Заявки магазинов">
    ...
    {active && <section className="packet-card request-draft-card">
      ...
      {active.status === "draft" ? <>
        ...
        <section className="request-comments" aria-label="Комментарии к печати"><div><span>КОММЕНТАРИИ</span><strong>До двух комментариев; категория печати настраивается в «Настройках печати»</strong></div>{([1, 2] as const).map(slot => <form key={slot} onSubmit={event => { event.preventDefault(); saveComment(slot); }}><label>Комментарий {slot}<textarea value={commentDrafts[slot].text} onChange={event => setCommentDrafts(current => ({ ...current, [slot]: { ...current[slot], text: event.target.value } }))} maxLength={2_000} placeholder="Например: привезти до открытия"/></label><button className="subtle-button" disabled={upsertRequestComment.isPending}><Save size={14}/>{upsertRequestComment.isPending ? "Сохраняем…" : "Сохранить"}</button></form>)}</section>
