...
export default function StoreRequests() {
  ...
  const canCloseRequest = Boolean(isManager || isAdmin);
  ...
  const [commentDrafts, setCommentDrafts] = useState<Record<number, { text: string; printCategoryGroupId: string }>>({
    1: { text: "", printCategoryGroupId: "" },
    2: { text: "", printCategoryGroupId: "" },
  });
  ...
  useEffect(() => {
    ...
    setCommentDrafts({
      1: { text: active.comments.find(comment => comment.slot === 1)?.text ?? "", printCategoryGroupId: String(active.comments.find(comment => comment.slot === 1)?.printCategoryGroupId ?? "") },
      2: { text: active.comments.find(comment => comment.slot === 2)?.text ?? "", printCategoryGroupId: String(active.comments.find(comment => comment.slot === 2)?.printCategoryGroupId ?? "") },
    });
  }, [active?.id]);
  ...
  const saveComment = (slot: 1 | 2) => {
    ...
    const printCategoryGroupId = Number(draft.printCategoryGroupId);
    if (!printCategoryGroupId) {
      toast.error("Выберите категорию печати для комментария");
      return;
    }
    upsertRequestComment.mutate({ requestId: active.id, slot, printCategoryGroupId, text: draft.text });
  };
  ...
  return <AuditShell kicker="30 / ЗАЯВКИ" title="Заявки магазинов">
    ...
    {active && <section className="packet-card request-draft-card">
      <div className="card-title request-draft-title"><div><span>{active.status === "draft" ? "ЧЕРНОВИК ЗАЯВКИ" : "ЗАКРЫТАЯ ЗАЯВКА"}</span><h3>{active.storeName} · {displayDate(active.businessDate)}</h3><small>№ {active.requestNumber} · {active.lines.length} поз.</small></div></div>
      {active.status === "draft" && <div className="request-actions">{canCloseRequest && active.lines.length > 0 && <ConfirmDangerDialog trigger={<button type="button" className="subtle-button" disabled={closeRequest.isPending || printRequests.isPending}><Printer size={14}/>{closeRequest.isPending || printRequests.isPending ? "Готовим…" : "Распечатать и закрыть"}</button>} title="Распечатать и закрыть заявку?" description="Заявка будет закрыта, после чего сформируется общая печать всех активных групп на эту дату. Изменение количества станет недоступно." confirmLabel="Распечатать и закрыть" disabled={closeRequest.isPending || printRequests.isPending} onConfirm={() => closeRequest.mutate({ requestId: active.id }, { onSuccess: () => printRequests.mutate({ businessDate: active.businessDate }) })}/>}<ConfirmDangerDialog trigger={<button type="button" className="subtle-button subtle-danger request-delete-draft" disabled={deleteDraft.isPending}><Trash2 size={14}/>{deleteDraft.isPending ? "Удаляем…" : "Удалить черновик"}</button>} title="Удалить черновик заявки?" description="Черновик и его строки будут удалены. Закрытые заявки и печатные подборки не изменятся; событие останется в общем журнале." confirmLabel="Удалить черновик" disabled={deleteDraft.isPending} onConfirm={() => deleteDraft.mutate({ requestId: active.id })}/></div>}
      {active.status === "draft" ? <>
        ...
        <section className="request-comments" aria-label="Комментарии к категориям печати"><div><span>КОММЕНТАРИИ</span><strong>До двух комментариев к печатным категориям</strong></div>{([1, 2] as const).map(slot => <form key={slot} onSubmit={event => { event.preventDefault(); saveComment(slot); }}><label>Комментарий {slot}<textarea value={commentDrafts[slot].text} onChange={event => setCommentDrafts(current => ({ ...current, [slot]: { ...current[slot], text: event.target.value } }))} maxLength={2_000} placeholder="Например: привезти до открытия"/></label><label>Напечатать в категории<ThemedSelect value={commentDrafts[slot].printCategoryGroupId} onChange={event => setCommentDrafts(current => ({ ...current, [slot]: { ...current[slot], printCategoryGroupId: event.target.value } }))}><option value="">Выберите категорию</option>{availableCategoryGroups.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</ThemedSelect></label><button className="subtle-button" disabled={upsertRequestComment.isPending}><Save size={14}/>{upsertRequestComment.isPending ? "Сохраняем…" : "Сохранить"}</button></form>)}</section>
