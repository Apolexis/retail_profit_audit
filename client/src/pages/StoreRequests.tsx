...
export default function StoreRequests() {
  ...
  const printGroups = trpc.inventoryRegistry.printGroups.useQuery(undefined, { enabled: canPrintRequests, retry: false });
  const printCategoryGroups = trpc.inventoryRegistry.printCategoryGroups.useQuery(undefined, { enabled: canPrintRequests, retry: false });
  ...
  return <AuditShell kicker="30 / ЗАЯВКИ" title="Заявки магазинов">
    ...
    {active && <section className="packet-card request-draft-card">
      <div className="card-title request-draft-title"><div><span>{active.status === "draft" ? "ЧЕРНОВИК ЗАЯВКИ" : "ЗАКРЫТАЯ ЗАЯВКА"}</span><h3>{active.storeName} · {displayDate(active.businessDate)}</h3><small>№ {active.requestNumber} · {active.lines.length} поз.</small></div>{active.status === "draft" && <div className="request-actions"><button type="button" className="subtle-button" onClick={startNewRequest}><X size={14}/>Отмена</button>{canCloseRequest && active.lines.length > 0 && <ConfirmDangerDialog trigger={<button type="button" className="subtle-button" disabled={closeRequest.isPending}><Send size={14}/>{closeRequest.isPending ? "Закрываем…" : "Закрыть заявку"}</button>} title="Закрыть заявку?" description="После закрытия позиции и количество нельзя изменить. Закрытые заявки доступны административному персоналу для печати по настроенным группам." confirmLabel="Закрыть заявку" disabled={closeRequest.isPending} onConfirm={() => closeRequest.mutate({ requestId: active.id })}/>}</div>}</div>
      {active.status === "draft" ? <>
    ...
    {canPrintRequests && <section className="packet-card request-print-config"><div className="card-title"><div><span>ПЕЧАТЬ ЗАКРЫТЫХ ЗАЯВОК</span><h3>Подборка по группам</h3></div><Printer size={20}/></div><p className="packet-note">В печать попадают только закрытые заявки выбранной даты и только точки, к которым есть доступ. Для режима «Отдельный лист на магазин» создается самостоятельный лист; для «Все магазины вместе» — одна общая подборка.</p><label className="request-print-date">Дата заявок<ExactDateControl value={businessDate} onChange={setBusinessDate} title="ДАТА ПЕЧАТИ ЗАЯВОК" ariaLabel="Выбрать дату заявок для печати"/></label><div className="request-print-selections"><fieldset><legend>Группы магазинов</legend><div>{activePrintGroups.map(group => <label key={group.id}><input type="checkbox" checked={printGroupIds.includes(group.id)} onChange={() => toggleSelection(group.id, printGroupIds, setPrintGroupIds)}/><span>{group.name}</span></label>)}</div></fieldset><fieldset><legend>Категории печати</legend><div>{availableCategoryGroups.map(group => <label key={group.id}><input type="checkbox" checked={printCategoryGroupIds.includes(group.id)} onChange={() => toggleSelection(group.id, printCategoryGroupIds, setPrintCategoryGroupIds)}/><span>{group.name}<small>{group.printMode === "per_store" ? "отдельный лист" : "все магазины вместе"}</small></span></label>)}</div></fieldset></div><div className="request-print-actions"><button type="button" className="subtle-button" disabled={!printGroupIds.length || !printCategoryGroupIds.length || printRequests.isPending} onClick={() => printRequests.mutate({ businessDate, printGroupIds, printCategoryGroupIds })}><Printer size={16}/>{printRequests.isPending ? "Готовим…" : "Распечатать заявки"}</button><small>Последняя подборка: {printProjection ? `${printProjection.sheets.length} листов` : "еще не сформирована"}</small></div></section>}
