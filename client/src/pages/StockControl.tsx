import { ArrowRight, Boxes, ClipboardCheck, PencilLine, Save, Search } from "lucide-react";
...
export default function StockControl() {
  ...
  return <AuditShell kicker="24 / ОСТАТКИ" title="Остатки">
    <section className="page-lede stock-lede"><div><span>УПРАВЛЕНИЕ МАГАЗИНАМИ</span><h2>Остатки магазинов</h2><p>Это отдельный рабочий список по точкам. Учетный остаток появляется только после закрытой инвентаризации или будущего подтвержденного движения. Отображается продажная цена назначенного виду цен и сумма по ней; внутренняя себестоимость не раскрывается.</p></div></section>
    <details className="packet-card stock-rule-disclosure"><summary><span>КАК ЧИТАТЬ ОСТАТКИ</span><strong>«Не посчитан» — это не ноль</strong></summary><div><p>Точный остаток не придумывается: пока позиция не прошла закрытую ревизию или подтвержденную корректировку, она остается со статусом «Не посчитан». Ноль появляется только после осознанного ввода количества.</p><p>Руководитель или администратор может изменить остаток прямо из строки: система сохраняет отдельное неизменяемое движение, разницу и обязательную причину в общем журнале. Продавец готовит пересчет, но не подтверждает прямую корректировку.</p></div></details>
