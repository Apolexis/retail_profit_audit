import { ArrowRight, Boxes, ClipboardCheck, Search } from "lucide-react";
...
export default function StockControl() {
  ...
  return <AuditShell kicker="24 / ОСТАТКИ" title="Остатки">
    <section className="page-lede stock-lede"><div><span>УПРАВЛЕНИЕ МАГАЗИНАМИ</span><h2>Остатки магазинов</h2><p>Это отдельный рабочий список по точкам. Учетный остаток появляется только после закрытой инвентаризации или будущего подтвержденного движения. Продажные цены, финансовые показатели и себестоимость здесь не выводятся.</p></div></section>
    <details className="packet-card stock-rule-disclosure"><summary><span>КАК ЧИТАТЬ ОСТАТКИ</span><strong>«Не посчитан» — это не ноль</strong></summary><div><p>Точный остаток не придумывается: пока позиция не прошла закрытую ревизию, она остается со статусом «Не посчитан». Ноль появится только после фактического пересчета и означает реальное отсутствие товара.</p><p>Чтобы исправить количество, откройте ревизию по нужной точке. Прямое редактирование учетного остатка исключено: после закрытия разница фиксируется отдельным движением и в общем журнале.</p></div></details>
