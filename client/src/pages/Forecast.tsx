...
export default function Forecast() {
  ...
  return <AuditShell kicker="19 / ПРОГНОЗ" title="Прогноз по фактам">
    ...
    <section className="forecast-controls"><label>Показатель<ThemedSelect value={metric} onChange={event => setMetric(event.target.value)}>{metricGroups.map(group => <optgroup key={group.label} label={group.label}>{group.metrics.map(item => <option key={item.code} value={item.code}>{item.label}</option>)}</optgroup>)}</ThemedSelect></label><div><span>Контур</span><strong>{scope}</strong><small>{selectedMetric.description}</small></div><div><span>Основа</span><strong>{forecast.historyYear} → {year}</strong><small>одинаковые завершенные месяцы</small></div></section>
