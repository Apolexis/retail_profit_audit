...
const metrics = [
  ...
  { code: "purchases", label: "Закупки", description: "товарный поток" },
  { code: "cash_operating_costs", label: "Траты нал", description: "операционные наличные расходы" },
...
export default function Forecast() {
  ...
  const [metric, setMetric] = useState<(typeof metrics)[number]["code"]>("revenue");
  ...
  return <AuditShell kicker="19 / ПРОГНОЗ" title="Прогноз по фактам">
    ...
    <section className="forecast-controls"><label>Показатель<select value={metric} onChange={event => setMetric(event.target.value as typeof metric)}>{metrics.map(item => <option key={item.code} value={item.code}>{item.label}</option>)}</select></label><div><span>Контур</span><strong>{scope}</strong><small>{selectedMetric.description}</small></div><div><span>Основа</span><strong>{forecast.historyYear} → {year}</strong><small>одинаковые завершенные месяцы</small></div></section>
