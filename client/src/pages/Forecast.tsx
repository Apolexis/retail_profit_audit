...
import { BarChart3, CalendarClock, ChartNoAxesCombined, Info, ReceiptText, TrendingDown, TrendingUp } from "lucide-react";
...
const metrics = [...primaryMetrics, ...metricGroups.flatMap(group => group.metrics)];
...
export default function Forecast() {
  const { range, rangeLabel, selectedStore } = useAudit();
  ...
  const evotorStoreIds = useMemo(() => {
    if (selectedStore === "__all__") return undefined;
    const store = (stores.data ?? []).find(item => item.name === selectedStore && !item.isHidden);
    return store ? [store.id] : [];
  }, [selectedStore, stores.data]);
  const evotorSales = trpc.inventoryRegistry.evotorSalesAnalytics.useQuery({ from: range.from, to: range.to, granularity: "month", storeIds: evotorStoreIds }, { retry: false });
  ...
  return <AuditShell kicker="19 / ПРОГНОЗ" title="Прогноз по фактам">
    ...
    <section className="forecast-controls"><label>Показатель<ThemedSelect value={metric} onChange={event => setMetric(event.target.value)}>{primaryMetrics.map(item => <option key={item.code} value={item.code}>{item.label}</option>)}{metricGroups.map(group => <optgroup key={group.label} label={group.label}>{group.metrics.map(item => <option key={item.code} value={item.code}>{item.label}</option>)}</optgroup>)}</ThemedSelect></label><div><span>Контур</span><strong>{scope}</strong><small>{selectedMetric.description}</small></div><div><span>Основа</span><strong>{forecast.historyYear} → {year}</strong><small>одинаковые завершенные месяцы</small></div></section>
    <section className="packet-card forecast-evotor-signal"><div className="card-title"><div><span><ReceiptText size={15}/> ОПЕРАЦИОННЫЙ СИГНАЛ ЭВОТОР</span><h3>Фактические чеки — отдельная база прогноза</h3></div><small>{scope} · {rangeLabel}</small></div>{evotorSales.isLoading ? <p className="packet-note">Проверяем нормализованные read-only чеки Эвотор…</p> : evotorSales.isError ? <p className="packet-note">Сигнал чеков временно недоступен. Финансовый прогноз не подменяется.</p> : evotorSales.data?.summary.checks ? <div className="forecast-evotor-kpis"><article><span>Чеки</span><strong>{new Intl.NumberFormat("ru-RU").format(evotorSales.data.summary.checks)}</strong></article><article><span>Сумма чеков</span><strong>{money(evotorSales.data.summary.amount)}</strong></article><article><span>Средний чек</span><strong>{money(evotorSales.data.summary.amount / evotorSales.data.summary.checks)}</strong></article></div> : <p className="packet-note">В выбранном финансовом периоде нет чеков Эвотор. {evotorSales.data?.coverage.from && evotorSales.data?.coverage.to ? `В read-only витрине доступен период ${evotorSales.data.coverage.from} — ${evotorSales.data.coverage.to}; он не подставляется в финансовый прогноз другого периода.` : "Витрина заполнится автоматически после загрузки чеков."}</p>}<p className="packet-note"><Info size={14}/> Этот блок не смешивает суммы чеков с P&L и не становится прогнозом прибыли: он показывает только независимый операционный факт для сверки с финансовой сезонностью.</p></section>
