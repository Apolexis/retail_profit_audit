...
describe("страница «База»", () => {
  ...
  it("помечает автоматические показатели и использует дату записи", () => {
    ...
    expect(page).toContain('import { moscowBusinessDate } from "@/lib/utils"');
    expect(page).toContain("const today = () => moscowBusinessDate()");
  ...
  it("называет общие наличные траты явно и меняет их позицию с НДФЛ 22%", () => {
    expect(page).toContain('"operating_costs", "cashless_operating_costs", "personal_income_tax_22", "driver_cashless"');
  ...
  it("не скрывает импортируемую безналичную расходную статью из селектора фактов", () => {
    expect(page).toContain('"operating_costs", "cashless_operating_costs", "personal_income_tax_22"');
    expect(page).toContain('cashless_operating_costs: "Расходы б/нал"');
  });
});
