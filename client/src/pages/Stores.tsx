...
export default function Stores() {
  ...
  const storeName = selectedStore !== "__all__" && facts.storeNames.includes(selectedStore) ? selectedStore : facts.storeNames[0];
  const store = facts.summaries.find(summary => summary.store === storeName) ?? facts.network;
  const rank = [...facts.summaries].sort((a, b) => b.netProfit - a.netProfit).findIndex(summary => summary.store === storeName) + 1;
  const monthly = facts.monthlyFor(storeName).map(row => ({
    month: row.month,
    revenue: row.revenue / 1000,
    grossProfit: row.grossProfit / 1000,
    netProfit: row.netProfit / 1000,
  }));
  const expenses = expenseDefinitions
    .map(([code, label]) => {
      const amount = Math.abs(store.expenseByCode[code] ?? 0);
      const share = store.revenue ? amount / store.revenue * 100 : 0;
      const medianShare = median(facts.summaries.map(summary => summary.revenue ? Math.abs(summary.expenseByCode[code] ?? 0) / summary.revenue * 100 : 0));
      return { code, label, amount, share, medianShare, gap: share - medianShare };
    })
  ...
  return (
    <AuditShell kicker="05 / ПРОФИЛИ ТОЧЕК" title="Магазин: полный разбор">
      {!facts.available ? (
        <section className="empty-state live-empty">
          <FileSpreadsheet size={30} />
          <h2>Нет магазина для разбора</h2>
          <p>После импорта Excel в профиле появится детальная аналитика выбранного магазина.</p>
        </section>
      ) : <>
        <section className="page-lede">
          <div>
            <h2>Одна точка — весь P&amp;L, товар, остаток и расходы.</h2>
            <p>Диапазон: <b>{rangeLabel}</b>. Ранги и медианы рассчитываются по доступным магазинам и импортированным фактам.</p>
          </div>
          <div className="page-controls">
            <label>Магазин
              <ThemedSelect value={storeName} onChange={event => setSelectedStore(event.target.value)}>
                {facts.storeNames.map(name => {
                  const item = facts.summaries.find(summary => summary.store === name);
                  const itemRank = [...facts.summaries].sort((a, b) => b.netProfit - a.netProfit).findIndex(summary => summary.store === name) + 1;
                  return <option key={name} value={name}>{name} · #{itemRank} по прибыли {item && item.netProfit < 0 ? "· риск" : ""}</option>;
                })}
              </ThemedSelect>
            </label>
          </div>
        ...
        <section className="packet-kpis equal">
          <article className="packet-kpi"><span>Выручка</span><strong>{money(store.revenue)}</strong><small>валовая маржа {formatPct(store.grossMargin)}</small></article>
        ...
        <section className="packet-split">
          <article className="packet-card wide">
            <div className="card-title"><div><span>P&amp;L · {storeName}</span><h3>Выручка, валовая и чистая прибыль</h3></div><small>млн / тыс. ₽</small></div>
            <MetricLineChart data={monthly} lines={[
              { key: "revenue", name: "Выручка", color: "#b694ff" },
              { key: "grossProfit", name: "Валовая прибыль", color: "#ffcf7b" },
              { key: "netProfit", name: "Чистая прибыль", color: "#ff6d8c" },
            ]} />
          </article>
          <article className="packet-card">
            <div className="card-title"><div><span>ТОВАРНЫЕ КАТЕГОРИИ</span><h3>Коп. и Мор. без приоритета</h3></div></div>
            <div className="product-ledger">
              <div><span>Коп. · закупка / продажа</span><strong>{money(store.purchaseSmoked)} / {money(store.salesSmoked)}</strong><small>наценка {formatPct(smokedMarkup)}</small></div>
              <div><span>Мор. · закупка / продажа</span><strong>{money(store.purchaseFrozen)} / {money(store.salesFrozen)}</strong><small>наценка {formatPct(frozenMarkup)}</small></div>
            </div>
            <div className="product-ledger">
              <div><span>Списания К.</span><strong>{money(store.writeoffSmoked)}</strong><small>{formatPct(store.revenue ? store.writeoffSmoked / store.revenue * 100 : 0)} выручки</small></div>
              <div><span>Списания М.</span><strong className="negative">{money(store.writeoffFrozen)}</strong><small>{formatPct(store.revenue ? store.writeoffFrozen / store.revenue * 100 : 0)} выручки · P&amp;L расход</small></div>
            </div>
          </article>
        </section>

        <section className="packet-card">
          <div className="card-title"><div><span>ВСЕ РАСХОДЫ · {storeName}</span><h3>Сумма, доля выручки, сетевая медиана</h3></div></div>
          <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Статья</th><th>Сумма</th><th>Доля</th><th>Медиана</th><th>Отклонение</th></tr></thead><tbody>{expenses.map(row => <tr key={row.code}><td>{row.label}</td><td>{money(row.amount)}</td><td>{formatPct(row.share)}</td><td>{formatPct(row.medianShare)}</td><td className={row.gap > 0 ? "negative" : "positive"}>{row.gap > 0 ? "+" : ""}{formatPct(row.gap)}</td></tr>)}</tbody><tfoot><tr className="table-total"><th scope="row">Итого</th><td>{money(expenseTotal)}</td><td>{formatPct(expenseShare)}</td><td>—</td><td>—</td></tr></tfoot></table></div>
        </section>
      </>}
    </AuditShell>
  );
