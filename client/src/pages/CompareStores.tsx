...
export default function CompareStores() {
  ...
  const aHasFacts = facts.rowsFor(effectiveLeft).some(row => row.monthDate >= from && row.monthDate <= to);
  const bHasFacts = facts.rowsFor(mode === "stores" ? effectiveRight : effectiveLeft).some(row => row.monthDate >= (mode === "stores" ? from : previousFrom) && row.monthDate <= (mode === "stores" ? to : previousTo));
  ...
  return <AuditShell kicker="06 / СРАВНЕНИЕ МАГАЗИНОВ" title="Магазины и периоды: одна рабочая плоскость">
    {facts.loading ? <FactsLoader/> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30}/><h2>Нет данных для сравнения</h2><p>Сначала подтвердите импорт книги Excel. Сохраненные ранее пары магазинов останутся в браузере.</p></section> : <>
      <section className="page-lede"><div><h2>Сравнивайте точки между собой или один магазин во времени.</h2><p>Пара магазинов сохраняется в браузере. В режиме «два магазина» используется активный календарный диапазон: <b>{rangeLabel}</b>. В режиме периода — такой же по длине предыдущий отрезок.</p></div><div className="page-controls compare-page-controls"><label>Режим<ThemedSelect value={mode} onChange={event => setMode(event.target.value as Mode)}><option value="stores">Два магазина</option><option value="periods">Один магазин · периоды</option></ThemedSelect></label><label className="compare-store-visibility"><span>Состав выбора</span><button type="button" className={onlyVisibleStores?"visibility-toggle active":"visibility-toggle"} aria-pressed={onlyVisibleStores} onClick={()=>setOnlyVisibleStores(value=>!value)}>{onlyVisibleStores?"Только видимые":"Все точки"}</button></label><label>Магазин A<ThemedSelect value={effectiveLeft} onChange={event => save("left", event.target.value)}>{names.map(name => <option key={name} value={name}>{name}</option>)}</ThemedSelect></label>{mode === "stores" && <label>Магазин B<ThemedSelect value={effectiveRight} onChange={event => save("right", event.target.value)}>{names.filter(name => name !== effectiveLeft).map(name => <option key={name} value={name}>{name}</option>)}</ThemedSelect></label>}<label className="compare-metric-control">Показатель<ThemedSelect value={metric} onChange={event => setMetric(event.target.value as ComparisonMetric)}>{metricGroups.map(group => <optgroup label={group.label} key={group.label}>{group.metrics.map(item => <option key={item} value={item}>{metaFor(item).label}</option>)}</optgroup>)}</ThemedSelect></label></div></section>
      <section className="packet-kpis equal"><article className={metricMeta.kind === "money" && aValue < 0 ? "packet-kpi risk" : "packet-kpi"}><span>{metricMeta.label} · {labelA}</span><strong className={metricMeta.kind === "money" ? aValue < 0 ? "negative" : aValue > 0 ? "positive" : "" : ""}>{aHasFacts ? displayValue(aValue, metricMeta.kind) : "—"}</strong><small>{mode === "stores" ? rangeLabel : "текущий срез"}</small></article><article className={metricMeta.kind === "money" && bValue < 0 ? "packet-kpi risk" : "packet-kpi"}><span>{metricMeta.label} · {labelB}</span><strong className={metricMeta.kind === "money" ? bValue < 0 ? "negative" : bValue > 0 ? "positive" : "" : ""}>{bHasFacts ? displayValue(bValue, metricMeta.kind) : "—"}</strong><small>{mode === "stores" ? rangeLabel : `${previousFrom} — ${previousTo}`}</small></article><article className={!aHasFacts || !bHasFacts ? "packet-kpi" : aValue - bValue < 0 ? "packet-kpi risk" : "packet-kpi"}><span>Разница A − B</span><strong className={!aHasFacts || !bHasFacts ? "" : aValue - bValue < 0 ? "negative" : aValue - bValue > 0 ? "positive" : ""}>{aHasFacts && bHasFacts ? `${aValue - bValue > 0 ? "+" : ""}${displayValue(aValue - bValue, metricMeta.kind)}` : "—"}</strong><small>по выбранному показателю</small></article><article className="packet-kpi"><span>{mode === "stores" ? "Ранг по прибыли" : "Магазин"}</span><strong>{mode === "stores" ? `#${rank(effectiveLeft)} / #${rank(effectiveRight)}` : effectiveLeft}</strong><small>{mode === "stores" ? `${effectiveLeft} / ${effectiveRight}` : "одна точка · два среза"}</small></article></section>
      <section className="packet-card"><div className="card-title"><div><span>ПО МЕСЯЦАМ · {metricMeta.label.toUpperCase()}</span><h3>{labelA} против {labelB}</h3></div><small>{metricMeta.kind === "percent" ? "%" : metricMeta.kind === "number" ? "дни" : "млн / тыс. ₽"}</small></div><MetricLineChart data={chart} percent={metricMeta.kind === "percent"} displayMode={chartMode} lines={[{key:labelA,name:labelA,color:"#ff6d8c"},{key:labelB,name:labelB,color:"#b694ff"}]}/><p className="packet-note control-comparison-note">Расчет фактический: значения собраны из продаж и закупок выбранного периода; разница показывает разрыв A − B, а медиана сети применяется на соответствующих сравнительных графиках. Символ «—» означает отсутствие факта, а не нулевое значение.</p></section>
