...
export default function CatalogProductEditor({ productId }: { productId?: number }) {
  ...
  useEffect(() => {
    if (!product) return;
    setName(product.canonicalName); setUnit(product.baseUnit); setVatRate(product.vatRate); setMarking(product.markingCategory); setCategoryName(product.evotorCategoryName?.trim() || "__none__"); setBarcodes(product.manualBarcodes || evotorBarcodes); setAlcoholCode(product.alcoholCode ?? ""); setAlcoholTypeCode(product.alcoholTypeCode ?? ""); setAlcoholStrengthPercent(product.alcoholStrengthPercent ?? ""); setVisibleInRequests(product.isVisibleInRequests); setEvotorExportEnabled(product.isEvotorExportEnabled); setInternalCostPrice(product.internalCostPrice ?? "");
  ...
  const saveCard = async (event: React.FormEvent) => {
    ...
    const alcohol = marking === "alcohol" || marking === "beer_marked" ? { alcoholCode, alcoholTypeCode, alcoholStrengthPercent: alcoholStrengthPercent.trim() ? Number(alcoholStrengthPercent) : null } : { alcoholCode: null, alcoholTypeCode: null, alcoholStrengthPercent: null };
