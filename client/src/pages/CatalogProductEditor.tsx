...
/** Documented Evotor product-type value for a future, separately approved export. */
const evotorProductTypeByMarking: Record<MarkingCategory, "NORMAL" | "ALCOHOL_MARKED"> = {
  none: "NORMAL", supplement: "NORMAL", seafood_caviar: "NORMAL", seafood_canned: "NORMAL",
  alcohol: "ALCOHOL_MARKED", beer_marked: "ALCOHOL_MARKED", beer_non_alcoholic: "NORMAL",
  soft_drinks: "NORMAL", water: "NORMAL", dairy: "NORMAL",
};
...
export default function CatalogProductEditor({ productId }: { productId?: number }) {
  ...
  const alcoholTypeOptions = useMemo(() => Array.from(new Set(((products.data ?? []) as CatalogProduct[]).map(item => item.alcoholTypeCode?.trim()).filter((item): item is string => Boolean(item)))).sort((left, right) => left.localeCompare(right, "ru", { numeric: true })), [products.data]);
