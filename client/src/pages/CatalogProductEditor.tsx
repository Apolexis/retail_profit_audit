import { ArrowLeft, Eye, EyeOff, Maximize2, Minimize2, Save, Tag } from "lucide-react";
...
export default function CatalogProductEditor({ productId }: { productId?: number }) {
  ...
  const saveCard = async (event: React.FormEvent) => {
    ...
    if (productId) update.mutate({ id: productId, canonicalName: name, baseUnit: unit, vatRate, catalogCategoryId: categoryId, markingCategory: marking, ...alcohol, manualBarcodes: barcodes, isVisibleInRequests: visibleInRequests, isEvotorExportEnabled: evotorExportEnabled });
    else {
      try {
        const created = await create.mutateAsync({ canonicalName: name, baseUnit: unit, vatRate, catalogCategoryId: categoryId, markingCategory: marking, ...alcohol, manualBarcodes: barcodes, isVisibleInRequests: visibleInRequests, isEvotorExportEnabled: evotorExportEnabled });
