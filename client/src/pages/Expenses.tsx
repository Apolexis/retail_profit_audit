...
export default function Expenses() {
  ...
  const trendTableTotals = trendLines.map(line => trendData.reduce((sum, row) => sum + Number((row as Record<string, string | number | undefined>)[line.key] ?? 0), 0));
  const cashLedgerTotal = cashLedger.reduce((sum, row) => sum + row.amount, 0);
