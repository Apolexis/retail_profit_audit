...
import { BenchmarkBars, MetricLineChart, StoreSeriesModeToggle, formatK, formatPct } from "@/components/AuditCharts";
...
import { businessWeekStart, formatBusinessCalendarDate, formatBusinessMonth } from "@/lib/utils";
...
const monthLabel = (month: string) => formatBusinessMonth(month.slice(0, 7), { month: "short", year: "numeric" });
type DetailLevel = "days" | "weeks" | "months";
const weekStart = (value: string) => businessWeekStart(value) ?? value;
const detailLabel = (key: string, level: DetailLevel) => { if (level === "months") return monthLabel(key); const date = formatBusinessCalendarDate(key, { day: "2-digit", month: "short" }); return level === "days" ? date : `Нед. ${date}`; };
...
export default function Expenses() {
  ...
  const [trendTableQuery, setTrendTableQuery] = useState("");
  const [cashTableQuery, setCashTableQuery] = useState("");
  ...
  const seriesStores = networkSelected ? facts.storeNames : selectedConcreteStores;
  ...
  const storeComparisonTrend = useMemo(() => {
    if (!showStoreSeries || seriesStores.length < 2 || selectedFields.length !== 1) return null;
    const field = selectedFields[0];
    const byPeriod = new Map<string, Record<string, number | string>>();
    seriesStores.forEach(storeName => aggregateTrend(facts.rowsFor(storeName), [field]).forEach(row => {
  ...
  }, [aggregateTrend, facts.rowsFor, selectedFields, seriesStores, showStoreSeries]);
  const trendData = storeComparisonTrend ?? monthlyTrend;
  const trendLines = storeComparisonTrend ? seriesStores.map((storeName, index) => ({ key: storeName, name: storeName, color: trendColors[index % trendColors.length] })) : selectedFields.map((field, index) => ({ key: field, name: nameFor(field), color: trendColors[index % trendColors.length] }));
  const trendTableRows = trendData.filter(row => {
    const values = row as Record<string, string | number | undefined>;
    return chartTableMatches(trendTableQuery, row.month, ...trendLines.map(line => values[line.key]));
  });
  const trendTableTotals = trendLines.map(line => trendTableRows.reduce((sum, row) => sum + Number((row as Record<string, string | number | undefined>)[line.key] ?? 0), 0));
  const filteredCashLedger = cashLedger.filter(row => chartTableMatches(cashTableQuery, row.name, row.amount, row.share));
  const cashLedgerTotal = filteredCashLedger.reduce((sum, row) => sum + row.amount, 0);
  const ledgerTotal = ledger.reduce((sum, row) => sum + row.amount, 0);
  ...
  return <AuditShell kicker="03 / РАСХОДНЫЙ АУДИТ" title="Расходы: статьи и магазины в одном срезе">
    {facts.loading ? <FactsLoader /> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30}/><h2>Нет фактов для расходного аудита</h2><p>Подтвердите импорт Excel: все доступные статьи расходов будут добавлены автоматически.</p></section> : <>
      ...
      <section className="packet-card"><div className="card-title"><div><span>ДИНАМИКА РАСХОДОВ · {scope}</span><h3>{detail === "days" ? "По дням" : detail === "weeks" ? "По неделям" : "Помесячно"}: {storeComparisonTrend ? `${primaryLabel} по магазинам` : selectedFields.length === 1 ? primaryLabel : `${selectedFields.length} статьи`}</h3></div><div className="chart-controls"><div className="chart-view-control" aria-label="Детализация расходов">{(["days", "weeks", "months"] as DetailLevel[]).map(level => <button type="button" key={level} className={detail === level ? "chart-view-button active" : "chart-view-button"} onClick={() => setDetail(level)}>{level === "days" ? "Дни" : level === "weeks" ? "Недели" : "Месяцы"}</button>)}</div>{seriesStores.length > 1 && selectedFields.length === 1 && <StoreSeriesModeToggle active={showStoreSeries} onChange={() => setShowStoreSeries(current => !current)}/>}<small>млн / тыс. ₽</small></div></div><MetricLineChart data={trendData} lines={trendLines}/></section>
      <section className="packet-card expense-detail-table"><div className="card-title"><div><span>ДАННЫЕ ПОД ГРАФИКОМ</span><h3>{storeComparisonTrend ? `${primaryLabel}: каждый магазин отдельно` : "Расходы выбранных магазинов"} по {detail === "days" ? "дням" : detail === "weeks" ? "неделям" : "месяцам"}</h3></div><small>млн / тыс. ₽</small></div><ChartTableSearch value={trendTableQuery} onChange={setTrendTableQuery} placeholder="Поиск интервала или значения" ariaLabel="Поиск в таблице расходов под графиком" shownRows={trendTableRows.length} totalRows={trendData.length}/><div className="data-table-wrap"><table className="data-table"><thead><tr><th>{detail === "days" ? "День" : detail === "weeks" ? "Неделя" : "Месяц"}</th>{trendLines.map(line => <th className="numeric-column" key={line.key}>{line.name}</th>)}</tr></thead><tbody>{trendTableRows.map(row => { const numericRow = row as Record<string, string | number | undefined>; return <tr key={String(row.month)}><td>{String(row.month)}</td>{trendLines.map(line => <td className="numeric-column" key={line.key}>{formatK(Number(numericRow[line.key] ?? 0))}</td>)}</tr>; })}</tbody><tfoot><tr className="table-total"><th scope="row">Итого</th>{trendTableTotals.map((value, index) => <td className="numeric-column" key={trendLines[index].key}>{formatK(value)}</td>)}</tr></tfoot></table></div></section>
      <section className="packet-card cash-breakdown-card"><div className="card-title"><div><span>СОСТАВ ТРАТ НАЛ · {scope}</span><h3>Какие наличные статьи формируют расход</h3></div><small>{detail === "days" ? "по дням" : detail === "weeks" ? "по неделям" : "помесячно"} · млн / тыс. ₽</small></div><MetricLineChart data={cashTrend} lines={cashCodes.map((code, index) => ({ key: code, name: nameFor(code), color: trendColors[index % trendColors.length] }))}/><ChartTableSearch value={cashTableQuery} onChange={setCashTableQuery} placeholder="Поиск наличной статьи" ariaLabel="Поиск в таблице наличных расходов" shownRows={filteredCashLedger.length} totalRows={cashLedger.length}/><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Наличная статья</th><th className="numeric-column">Сумма</th><th className="numeric-column">Доля выручки</th></tr></thead><tbody>{filteredCashLedger.map(row => <tr key={row.code}><td>{row.name}</td><td className="numeric-column">{money(row.amount)}</td><td className="numeric-column">{formatPct(row.share)}</td></tr>)}</tbody><tfoot><tr className="table-total"><th scope="row">Итого</th><td className="numeric-column">{money(cashLedgerTotal)}</td><td className="numeric-column">{formatPct(share(cashLedgerTotal, store.revenue))}</td></tr></tfoot></table></div><div className="expense-mobile-ledger" aria-label="Наличные статьи: мобильная версия">{filteredCashLedger.map(row => <article key={row.code}><strong>{row.name}</strong><dl><div><dt>Сумма</dt><dd>{money(row.amount)}</dd></div><div><dt>Доля выручки</dt><dd>{formatPct(row.share)}</dd></div></dl></article>)}<article className="table-total"><strong>Итого</strong><dl><div><dt>Сумма</dt><dd>{money(cashLedgerTotal)}</dd></div><div><dt>Доля выручки</dt><dd>{formatPct(share(cashLedgerTotal, store.revenue))}</dd></div></dl></article></div></section>
      ...
      <section className="packet-card expense-ledger-card"><div className="card-title"><div><span>ПОЛНЫЙ РАСХОДНЫЙ P&amp;L · {scope}</span><h3>Сумма, доля выручки и сетевой бенчмарк</h3></div><small>млн / тыс. ₽</small></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Статья</th><th className="numeric-column">Сумма</th><th className="numeric-column">Доля</th><th className="numeric-column">Медиана</th><th className="numeric-column">Отклонение</th></tr></thead><tbody>{ledger.map(row => <tr key={row.code}><td>{row.name}</td><td className="numeric-column">{money(row.amount)}</td><td className="numeric-column">{formatPct(row.share)}</td><td className="numeric-column">{formatPct(row.medianShare)}</td><td className={row.gap > 0 ? "numeric-column negative" : "numeric-column positive"}>{row.gap > 0 ? "+" : ""}{formatPct(row.gap)}</td></tr>)}</tbody><tfoot><tr className="table-total"><th scope="row">Итого</th><td className="numeric-column">{money(ledgerTotal)}</td><td className="numeric-column">{formatPct(share(ledgerTotal, store.revenue))}</td><td className="numeric-column">—</td><td className="numeric-column">—</td></tr></tfoot></table></div><div className="expense-mobile-ledger" aria-label="Полный расходный P&L: мобильная версия">{ledger.map(row => <article key={row.code}><strong>{row.name}</strong><dl><div><dt>Сумма</dt><dd>{money(row.amount)}</dd></div><div><dt>Доля</dt><dd>{formatPct(row.share)}</dd></div><div><dt>Медиана</dt><dd>{formatPct(row.medianShare)}</dd></div><div><dt>Отклонение</dt><dd className={row.gap > 0 ? "negative" : "positive"}>{row.gap > 0 ? "+" : ""}{formatPct(row.gap)}</dd></div></dl></article>)}<article className="table-total"><strong>Итого</strong><dl><div><dt>Сумма</dt><dd>{money(ledgerTotal)}</dd></div><div><dt>Доля</dt><dd>{formatPct(share(ledgerTotal, store.revenue))}</dd></div></dl></article></div></section>
    </>}
  </AuditShell>;
}
