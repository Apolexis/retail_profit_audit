...
describe("страница «Прайс‑контроль»", () => {
	it("отделяет поисковый инпут имён связей от списка результатов", () => {
		expect(styles).toContain(".packet .price-link-summary .price-directory-search { margin-bottom: 10px; }");
	});
	...
	it("не навязывает поиск в коротком списке и оставляет поиск для длинного", () => {
	  expect(page).toContain("searchable={options.length > 10}");
	  expect(page).toContain('searchPlaceholder="Найти в списке"');
	  expect(page).not.toContain("autoFocus");
	});

	it("показывает итог текущего сравнения без ложного сложения экономии", () => {
	  expect(page).toContain("const comparisonTotals = useMemo");
	  expect(page).toContain('className="price-comparison-total"');
	  expect(page).toContain("ИТОГО");
	  expect(page).toContain("comparisonTotals.products");
	  expect(page).toContain("comparisonTotals.offers");
	  expect(styles).toContain(".packet .price-comparison-total");
	});
  ...
  it("сохраняет работу с ранее импортированными файлами", () => {
    ...
    expect(page).not.toContain('type="date"');
		...
		it("показывает весь сохраненный импорт с одним явным сохранением строки", () => {
    ...
    expect(page).toContain("editingSavedImportRowId");
			...
			expect(page).toContain("saveSavedImportRow");
			expect(page).toContain('<Save size={14} /> Сохранить');
			expect(page).not.toContain("Сохранить связь");
			expect(page).not.toContain("Сохранить категорию");
			expect(page).not.toContain(">Готово</button>");
			expect(page).not.toContain("savedImportLinkSearches");
			expect(styles).toContain(".packet .price-saved-import-row");
		});

		it("сохраняет выбранную дату сразу и дает явно закрыть предпросмотр", () => {
			expect(page).toContain('ariaLabel="Изменить дату сохраненного прайс-листа"');
			expect(page).toContain("if (sourceDate === selectedImportDate || updateImportDate.isPending) return;");
			expect(page).toContain('aria-label="Закрыть предпросмотр сохраненного прайс-листа"');
			expect(page).toContain('setSelectedImportId(null)');
			expect(page).not.toContain("Сохранить дату");
			expect(styles).toContain(".price-import-preview-close");
		});

		it("дает явно подписанное удаление рядом с равным действием изменения", () => {
			expect(page).toContain('title="Удалить позицию из сохраненного прайса"');
			expect(page).toContain("<Trash2 size={15} />");
			expect(page).toContain("Удалить");
			expect(styles).toContain("grid-template-columns: repeat(2, minmax(92px, 1fr));");
			expect(styles).toContain(".price-saved-import-actions .price-preview-remove");
		});

		it("оставляет обязательность у текста поля, а не скрытой подсказкой", () => {
			expect(page).toContain("Наименование товара <em>*</em>");
			expect(page).toContain("Имя связи <em>*</em>");
			expect(styles).toContain(".price-directory-editor label > em");
		});

		it("не смешивает в тёмной теме сине-серые поля с коралловыми селекторами", () => {
			expect(styles).toContain('html[data-audit-theme="dark"] .packet :is(.price-comparison, .price-history, .price-directory, .price-upload, .price-import-guide, .price-import-preview) :is(input, textarea)');
			expect(styles).toContain("background: #25141e !important;");
			expect(styles).toContain("border-color: #754456 !important;");
			expect(styles).toContain(":is(input, textarea):not(:focus):not(:focus-visible)");
		});

		it("строит поиск справочника единой поверхностью и не сжимает его рядом с фильтром", () => {
			expect(page).toContain('placeholder="Товар, код или категория"');
			expect(page).toContain('aria-label="Поиск товаров по названию, коду или категории"');
			expect(styles).toContain('.packet .price-directory { container-type: inline-size; }');
			expect(styles).toContain('grid-template-columns: minmax(270px, 1fr) minmax(220px, .9fr);');
			expect(styles).toContain('.packet .price-directory-toolbar > small { grid-column: 1 / -1;');
			expect(styles).toContain('@container (max-width: 620px)');
			expect(styles).toContain('html[data-audit-theme="light"] .packet .price-directory-search > input { border-color: transparent !important; background: transparent !important;');
			expect(styles).toContain('html[data-audit-theme="dark"] .packet .price-directory-search > input { border-color: transparent !important; background: transparent !important;');
			expect(styles).toContain('html[data-audit-theme="light"] .packet .price-directory .price-directory-search > input {');
			expect(styles).toContain('html[data-audit-theme="dark"] .packet .price-directory .price-directory-search > input {');
			expect(styles).toContain('html[data-audit-theme="dark"] .packet .price-directory .price-directory-search > input:not(:focus):not(:focus-visible),');
		...
		expect(styles).toContain("grid-template-columns: minmax(0, 1.36fr) minmax(215px, .84fr)");
		expect(styles).toContain(".price-preview-row .price-preview-product { grid-column: 1; grid-row: 1 / span 2;");
		expect(styles).toContain(".price-preview-table > div.price-preview-row,");
		expect(styles).toContain("grid-template-columns: minmax(0, 1fr);");
		});

		it("не оставляет серые или чисто-белые поля и обычные действия в светлом прайс-контроле", () => {
			expect(styles).toContain("--price-surface: #f2f8ff;");
			expect(styles).toContain("background: #f2f8ff !important;");
			expect(styles).toContain("background: #e6f2ff !important;");
			expect(styles).toContain("border-color: #0a84ff !important;");
		});

		it("центрирует поиск над фильтрами и держит действия предложения в одной строке", () => {
			expect(styles).toContain(".packet .price-toolbar > label:first-child");
			expect(styles).toContain("width: min(470px, 100%);");
			expect(styles).toContain("justify-self: center;");
			expect(styles).toContain(".packet .price-offer-actions");
			expect(styles).toContain("grid-template-columns: repeat(2, minmax(0, 1fr));");
			expect(styles).toContain(".price-offer-edit-action");
			expect(styles).toContain("justify-content: center;");
		});

		it("изолирует поиск товаров от белого контура и использует тематичный hover", () => {
			expect(page).toContain('className="price-directory-search price-directory-product-search"');
			expect(styles).toContain('.price-directory-product-search:focus-within');
			expect(styles).toContain('background: #e6f2ff !important;');
			expect(styles).toContain('background: #25141e !important;');
			expect(styles).not.toContain('.price-directory-product-search:focus-within { border-color: #ffffff');
		});
  ...
  it("открывает только выбранный сохраненный прайс, фильтрует список по поставщику и выдает его порциями", () => {
    ...
    expect(styles).toContain(".packet .price-import-controls");
		...
		it("дает искать имя связи внутри одного раскрытия без дублирующего поля в preview и сопоставлении", () => {
			expect(page).toContain("const productLinkOptions = (selectedValue = \"\") =>");
			expect(page).toContain("group.linkCode");
			expect(page).toContain("group.canonicalName");
			expect(page).toContain("searchable");
			expect(page).not.toContain("previewProductLinkSearches");
			expect(page).not.toContain("linkSearches");
			expect(page).not.toContain("price-product-link-search");
		});
  ...
  it("создает редактируемые имена связей для ранее сохраненных несвязанных строк", () => {
    ...
    expect(page).toContain('searchPlaceholder="Найти в списке"');
    expect(page).not.toContain("Поиск имени связи или кода");
    expect(styles).toContain(".packet .price-alias-group:hover");
  });

  it("выбирает товар внутри имени связи через компактный селект без отдельной цены", () => {
    expect(page).toContain("price-link-product-chooser");
    expect(page).toContain("Товар с предложениями и ценами");
    expect(page).toContain('placeholder="Выберите товар"');
    expect(page).toContain("options={selected.products.map(product");
    expect(page).not.toContain("Добавить цену товару");
    expect(page).not.toContain("ДОБАВИТЬ ЦЕНУ В ИСТОРИЮ");
    expect(page).not.toContain("comparisonManualOffer");
    expect(styles).toContain(".packet .price-link-product-chooser { display: grid;");
  });

	it("оставляет в tooltip истории только название позиции и цену", () => {
	    const tooltip = page.split("function PriceHistoryTooltip")[0];
	    expect(page).toContain("function PriceOfferTooltipDetails");
	    expect(page).toContain("function PriceHistoryTooltip");
	    expect(tooltip).toContain("<strong>{offer.rawName}</strong>");
	    expect(tooltip).toContain("<b>{priceLabel(offer)}</b>");
	    expect(tooltip).not.toContain("price-offer-tooltip-details");
	    expect(tooltip).not.toContain("<dt>Город</dt>");
	    expect(tooltip).not.toContain("<dt>Условие</dt>");
	    expect(tooltip).not.toContain("<dt>Исходная строка</dt>");
	    expect(tooltip).not.toContain("<dt>Фасовка</dt>");
	    expect(tooltip).not.toContain("<dt>Производитель</dt>");
	    expect(tooltip).not.toContain("<dt>Состав места</dt>");
	    expect(tooltip).not.toContain("<PriceChangeBadge change={offer.priceChange}");
	    ...
	    expect(styles).toContain("max-height: min(224px, calc(100dvh - 32px))");
	  });
  ...
  it("использует знакомую двухколоночную панель импорта и тематичные состояния полей", () => {
    ...
    expect(styles).toContain(".packet .price-import-workbench.is-preview-ready");
		expect(styles).toContain(".packet .price-import-workbench > :is(.price-upload, .price-import-guide) { background: var(--price-card); border-color: var(--price-border) !important; }");
  ...
  it("не выводит мобильную таблицу и график за границы выбранной области", () => {
    ...
    expect(styles).toContain(".packet .price-link-product-chooser .price-select-trigger { width: 100%; min-width: 0; }");
  });
  ...
  it("показывает изменение цены относительно предыдущего предложения", () => {
	...
	it("использует разные тематические акценты: iOS‑синий в светлой и коралловый в темной", () => {
		expect(styles).toContain('html[data-audit-theme="light"] .packet { --price-accent: #0a84ff');
		expect(styles).toContain('html[data-audit-theme="dark"] .packet { --price-accent: #ff765f');
  ...
  it("сохраняет видимый неменяющий геометрию hover-контур карточки связи", () => {
    expect(page).toContain('className="price-alias-list"');
    expect(page).toContain('className="price-alias-group"');
    expect(styles).toContain(".packet .price-alias-list > .price-alias-group:hover");
    expect(styles).toContain("box-shadow: 0 0 0 1px color-mix(in srgb, var(--price-accent) 16%, transparent);");
