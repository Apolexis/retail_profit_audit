...
export default function Pricing() {
  ...
  const storeMonthly = useMemo(() => {
    ...
    facts.storeNames.forEach(storeName => facts.monthlyFor(storeName).forEach(row => {
      const point = byMonth.get(row.month) ?? { month: row.month };
      point[storeName] = markup(row, category);
      byMonth.set(row.month, point);
    }));
    return Array.from(byMonth.values());
  }, [all, showStoreSeries, facts.monthlyFor, facts.storeNames, category]);
  const chartData = storeMonthly ?? monthly;
  const chartLines = storeMonthly ? facts.storeNames.map((storeName, index) => ({ key: storeName, name: storeName, color: storeColors[index % storeColors.length] })) : [{ key: "% наценки Коп.", name: "% наценки Коп.", color: "#ff6d8c" }, { key: "% наценки Мор.", name: "% наценки Мор.", color: "#b694ff" }, { key: "% наценки Общий", name: "% наценки Общий", color: "#ffcf7b" }];
  ...
  const markupDetails = useMemo(() => (Object.keys(info) as Category[]).map(kind => { const selectedPairForKind = pair(selected, kind); const selectedMarkup = markup(selected, kind); const networkMedian = median(facts.summaries.map(summary => markup(summary, kind))); return { kind, selectedPair: selectedPairForKind, selectedMarkup, networkMedian, gap: selectedMarkup - networkMedian }; }), [facts.summaries, selected]);
  ...
  return <AuditShell kicker="02 / ЦЕНОВОЙ АУДИТ" title="Цены: категории и общий товарный поток">
    {facts.loading ? <FactsLoader /> : !facts.available ? <section className="empty-state live-empty"><FileSpreadsheet size={30} /><h2>Нет товарных данных для ценового аудита</h2><p>После импорта книги станут доступны закупки и продажи Коп. и Мор. по каждому магазину и месяцу.</p></section> : <>
      <section className="page-lede"><div><h2>Смотрите наценку вместе с объемом и списаниями.</h2><p>Срез: <b>{rangeLabel}</b>. Наценка рассчитывается из фактических продаж и закупок: <b>(продажи − закупки) ÷ закупки × 100</b>. Для нескольких дат используется взвешенный расчет, а не сумма процентов; ценовой коридор — сигнал для пилота, не автоматическое изменение цены.</p></div><div className="page-controls"><label>Срез<ThemedSelect value={all ? "__all__" : selectedStore} onChange={event => setSelectedStore(event.target.value)}><option value="__all__">Все магазины</option>{facts.storeNames.map(name => <option key={name} value={name}>{name}</option>)}</ThemedSelect></label><label>Категория<ThemedSelect value={category} onChange={event => setCategory(event.target.value as Category)}><option value="all">% наценки Общий</option><option value="smoked">% наценки Коп.</option><option value="frozen">% наценки Мор.</option></ThemedSelect></label></div></section>
      ...
      <section className="packet-split pricing-split pricing-chart-stack"><article className="packet-card wide"><div className="card-title"><div><span>ЦЕНОВОЙ КОРИДОР · {info[category].title.toUpperCase()}</span><h3>Все магазины по наценке</h3></div><small>%</small></div><BenchmarkBars data={market} median={medianMarkup} selectedLabel={scopeLabel} /></article><article className="packet-card pricing-monthly-card"><div className="card-title"><div><span>ДИНАМИКА · {scopeLabel}</span><h3>{storeMonthly ? "Наценка: каждый магазин отдельно" : "Наценка по месяцам"}</h3></div><div className="chart-controls">{all && <StoreSeriesModeToggle active={showStoreSeries} onChange={() => setShowStoreSeries(current => !current)} />}<small>%</small></div></div><MetricLineChart data={chartData} percent lines={chartLines} /><div className="pricing-monthly-facts"><div><span>Последний месяц с наценкой</span><b>{latest?.month ?? "Нет данных"}</b></div><div><span>Изменение к прошлому месяцу</span><b className={markupDelta === null ? "" : markupDelta < 0 ? "negative" : markupDelta > 0 ? "positive" : ""}>{markupDelta === null ? "Нет базы" : `${markupDelta > 0 ? "+" : ""}${formatPct(markupDelta)}`}</b></div><div><span>Отклонение от медианы сети</span><b className={medianGap === null ? "" : medianGap < 0 ? "negative" : medianGap > 0 ? "positive" : ""}>{medianGap === null ? "Нет данных" : `${medianGap > 0 ? "+" : ""}${formatPct(medianGap)}`}</b></div></div><div className={`pricing-recommendation ${recommendation.tone}`}><span>РЕКОМЕНДАЦИЯ ПО ФАКТАМ</span><h4>{recommendation.title}</h4><p>{recommendation.detail}</p></div></article></section>
      <section className="packet-card pricing-markup-detail"><div className="card-title"><div><span>ДЕТАЛИЗАЦИЯ НАЦЕНКИ · {scopeLabel.toUpperCase()}</span><h3>Копченая, мороженая и общий товарный поток</h3><small>Сравнение показывает фактический расчет, его разрыв с медианой сети и базу продаж/закупок.</small></div></div><div className="pricing-markup-detail-grid">{markupDetails.map(item => <article key={item.kind}><span>% НАЦЕНКИ · {info[item.kind].title.toUpperCase()}</span><strong>{formatPct(item.selectedMarkup)}</strong><small>медиана сети: {formatPct(item.networkMedian)}</small><b className={item.gap < 0 ? "negative" : item.gap > 0 ? "positive" : ""}>{item.gap > 0 ? "+" : ""}{formatPct(item.gap)} к медиане</b><div><span>Продажи {money(item.selectedPair.sales)}</span><span>Закупки {money(item.selectedPair.purchase)}</span></div></article>)}</div></section>
    </>}
  </AuditShell>;
}
