...
export default function Pricing() {
  ...
  const markupDetails = useMemo(() => (Object.keys(info) as Category[]).map(kind => {
    const selectedPairForKind = pair(selected, kind);
    const selectedMarkup = markup(selected, kind);
    const networkMedian = median(facts.summaries.map(summary => markup(summary, kind)));
    return { kind, selectedPair: selectedPairForKind, selectedMarkup, networkMedian, gap: selectedMarkup - networkMedian };
  }), [facts.summaries, selected]);
