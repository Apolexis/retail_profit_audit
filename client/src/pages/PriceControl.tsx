...
function PriceOfferTooltipDetails({ offer }: { offer: PriceOfferTooltipDatum }) {
  return <div className="price-offer-tooltip-main">
      <strong>{offer.rawName}</strong>
...
export default function PriceControl({
  ...
  const [comparisonLinkSearch, setComparisonLinkSearch] = useState("");
  ...
  const productLinkOptions = (selectedValue = "") => {
  ...
  const filteredProductComparisons = useMemo(() => {
  ...
  }, [overview.data?.comparisons, categoryFilter, characteristicFilter, supplierFilter, offerMarketFilter, visibilityFilter]);
    ...
    return Array.from(groups.values()).map(entry => {
	  ...
	  const visibleComparisons = useMemo(() => {
	    const query = comparisonLinkSearch.trim().toLocaleLowerCase("ru");
	    if (!query) return filteredComparisons;
	    return filteredComparisons.filter(item => `${item.product.canonicalName} ${item.product.internalCode}`.toLocaleLowerCase("ru").includes(query));
	  }, [filteredComparisons, comparisonLinkSearch]);
	  const comparisonTotals = useMemo(() => visibleComparisons.reduce((total, item) => ({
	    links: total.links + 1,
	    products: total.products + item.products.length,
	    offers: total.offers + item.offers.length,
	  }), { links: 0, products: 0, offers: 0 }), [visibleComparisons]);
	  const selected =
    visibleComparisons.find(item => item.product.id === selectedLinkGroupId) ??
    visibleComparisons.find(item => item.products.some(product => product.id === selectedProductId)) ??
  ...
  const saveSavedImportRow = async ({
    rowId,
    rawName,
    rawCategory,
    target,
    ...
    const selectedProductId = Number(target);
    const draftedName = newNames[rowId]?.trim();
    const draftedCategory = savedImportCategoryDrafts[rowId];
    const categoryChanged = draftedCategory !== undefined && draftedCategory.trim() !== (rawCategory ?? "");

    try {
      if (selectedProductId) {
        await linkRow.mutateAsync({ rowId, productId: selectedProductId, saveAlias: true });
      } else if (draftedName) {
        await createAndLink(rowId, rawName, rawCategory);
      }
      if (categoryChanged && draftedCategory.trim()) {
        await updateImportRowCategory.mutateAsync({ rowId, category: draftedCategory.trim() });
      }
      setEditingSavedImportRowId(null);
    } catch {
      /* mutation shows the Russian safe error; keep the editor open for correction */
    }
  };
            ...
            {overview.isLoading ? (
              ...
              <div className="price-comparison-body">
	                <aside className="price-product-panel">
	                  ...
	                  <div className="price-comparison-total" aria-label="Итог текущего сравнения">
	                    <span>ИТОГО</span>
	                    <strong>{comparisonTotals.links} {comparisonTotals.links === 1 ? "связь" : "связи"} · {comparisonTotals.products} {comparisonTotals.products === 1 ? "товар" : "товаров"} · {comparisonTotals.offers} {comparisonTotals.offers === 1 ? "предложение" : "предложений"}</strong>
	                  </div>
	                  <label className="price-comparison-search">
	                    Найти имя связи
	                    <input
	                      value={comparisonLinkSearch}
	                      onChange={event => {
	                        setComparisonLinkSearch(event.target.value);
	                        setSelectedLinkGroupId(null);
	                        setSelectedProductId(null);
	                      }}
                  ...
                  <div className="price-detail">
                    <div className="price-detail-heading">
                      <div>
                        ...
                        <label className="price-link-product-chooser">
                          <span>Товар с предложениями и ценами</span>
                          <PriceSelect
                            value={selectedPriceProduct ? String(selectedPriceProduct.id) : ""}
                            onValueChange={productId => setSelectedProductId(Number(productId))}
                            placeholder="Выберите товар"
                            options={selected.products.map(product => ({ value: String(product.id), label: product.canonicalName }))}
                          />
                        </label>
                      </div>
                          ...
                          <label className="price-preview-product-link">
                            ...
                            <PriceSelect
                              ...
                              options={[
                                ...
                                ...productLinkOptions(previewProductLinks[index] ?? ""),
                              ]}
                            />
                            {!linkedProduct && (
                              <input
          ...
          {selectedImport && (
            <section ref={savedImportPreviewRef} className="packet-card price-import-preview">
              <div className="card-title">
                ...
                <button
                  type="button"
                  className="packet-link compact subtle price-import-preview-close"
                  onClick={() => {
                    setEditingSavedImportRowId(null);
                    setSelectedImportId(null);
                  }}
                  aria-label="Закрыть предпросмотр сохраненного прайс-листа"
                >
                  <X size={14} /> Закрыть
                </button>
              </div>
                ...
                <div className="price-import-date-control">
                  ...
                  {canEdit ? (
                    <ExactDateControl
                      ...
                      onChange={sourceDate => {
                        if (sourceDate === selectedImportDate || updateImportDate.isPending) return;
                        ...
                        <div className="price-saved-import-editor">
                          ...
                          <div className="price-saved-import-editor-actions">
                            <button
                              type="button"
                              className="packet-link compact"
                              disabled={linkRow.isPending || updateImportRowCategory.isPending || createProduct.isPending}
                              onClick={() => void saveSavedImportRow({
                                rowId: row.rowId,
                                rawName: row.rawName,
        ...
        <>
          ...
          <section className="packet-card price-directory">
            ...
            <div className="price-directory-toolbar" role="search">
              <label className="price-directory-search price-directory-product-search">
                <Search size={15} aria-hidden="true" />
                <input
                  value={directoryProductSearch}
                  onChange={event => setDirectoryProductSearch(event.target.value)}
                  placeholder="Товар, код или категория"
                  aria-label="Поиск товаров по названию, коду или категории"
                />
              </label>
              <PriceSelect
          ...
          {directoryTab === "links" && canEdit && filteredUnmappedRows.length > 0 && (
            <section className="packet-card price-mapping">
              ...
              <div className="price-links-toolbar">
                <small>
                  Нераспознано: {filteredUnmappedRows.length}
                </small>
          ...
          {directoryTab === "links" && canEdit && (
            <section className="packet-card price-aliases">
              ...
              <div className="price-link-summary" role="search">
                <span>Итого связей: {filteredLinkGroups.length}</span>
                <label className="price-directory-search">
                  <Search size={15} aria-hidden="true" />
                  <input
