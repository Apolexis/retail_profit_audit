...
function maskPhone(phone: string | undefined) {
  ...
  return digits.length >= 4 ? `+7 ••• •••-${digits.slice(-4)}` : "—";
}
...
export default function Profile() {
  ...
  const change = trpc.localAuth.changePassword.useMutation({
    onSuccess: () => { setCurrentPassword(""); setNextPassword(""); toast.success("Пароль изменен"); },
  ...
  const passkeys = trpc.localAuth.passkeys.useQuery(undefined, { retry: false });
  ...
  const enablePasskey = async () => {
    try {
      ...
      toast.error(passkeyErrorText(error), { duration: 9000 });
    }
  };
  ...
  return <AuditShell kicker="14 / ПРОФИЛЬ" title="Профиль и безопасность">
    <section className="page-lede"><div><span>ЛОКАЛЬНАЯ УЧЕТНАЯ ЗАПИСЬ</span><h2>Профиль доступа</h2><p>Здесь можно изменить пароль, настроить телефонные уведомления и завершить текущую сессию. Администратор управляет системой целиком; аналитик видит только назначенные ему магазины.</p></div></section>
    <section className="access-grid">
      <article className="packet-card"><span className="card-eyebrow">ТЕКУЩАЯ РОЛЬ</span><h3>{isAdmin ? "Администратор" : "Аналитик"}</h3><p>Номер телефона: <strong>{maskPhone(account?.phone)}</strong></p><p className="packet-note">{isAdmin ? "Доступ ко всем магазинам, импорту, базе, журналу и настройке пользователей." : "Доступ к каждому магазину задается отдельно: «Просмотр» или «Редактирование»."}</p><button className="subtle-button" onClick={() => logout.mutate()}>Выйти из системы</button></article>
