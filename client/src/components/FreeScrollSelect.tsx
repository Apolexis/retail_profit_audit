...
export function FreeScrollSelect({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} modal={false}>
      ...
      >
        {searchable && <label className="free-scroll-select-search"><span className="sr-only">{searchPlaceholder}</span><input ref={searchInputRef} value={search} onChange={event => setSearch(event.target.value)} onPointerDownCapture={event => event.stopPropagation()} onClickCapture={event => event.stopPropagation()} onKeyDownCapture={event => event.stopPropagation()} onKeyUpCapture={event => event.stopPropagation()} onFocusCapture={event => event.stopPropagation()} placeholder={searchPlaceholder}/></label>}
        <button
          type="button"
