...
export function DateRangeControl({compact=false,value,onChange,title="ПЕРИОД АНАЛИЗА",ariaLabel="Изменить период анализа"}:Props){
  ...
  const pickDay=(day:Date,source:"click"|"touch"="click")=>{const dayKey=day.getTime();if(source==="click"&&touchDayRef.current===dayKey&&Date.now()-touchHandledAtRef.current<700){touchDayRef.current=null;return}if(!draft?.from||draft.to){setDraft({from:day});setHovered(undefined);return}const [from,to]=day<draft.from?[day,draft.from]:[draft.from,day];applyRange({from:toIso(from),to:toIso(to)})};
  const pickTouchDay=(event:React.PointerEvent<HTMLButtonElement>,day:Date)=>{if(event.pointerType!=="touch")return;event.preventDefault();touchDayRef.current=day.getTime();touchHandledAtRef.current=Date.now();pickDay(day,"touch")};
  const moscowDayKey=moscowBusinessDate(); const year=Number(moscowDayKey.slice(0,4));
  const shortcuts=useMemo<Shortcut[]>(()=>{const anchor=parseISO(`${moscowDayKey}T12:00:00`);const currentMonthStart=startOfMonth(anchor);const previousMonthStart=startOfMonth(subMonths(anchor,1));const previousMonthEnd=endOfMonth(previousMonthStart);const currentWeekStart=startOfWeek(anchor,{weekStartsOn:1});const previousWeekStart=startOfWeek(subWeeks(anchor,1),{weekStartsOn:1});const completedYtd=anchor.getMonth()===0?[]:[{label:"Год до конца прошлого месяца",from:`${year}-01-01`,to:toIso(previousMonthEnd),wide:true}];return [{label:"Сегодня",from:moscowDayKey,to:moscowDayKey},{label:"Эта неделя",from:toIso(currentWeekStart),to:toIso(endOfWeek(anchor,{weekStartsOn:1}))},{label:"Прошлая неделя",from:toIso(previousWeekStart),to:toIso(endOfWeek(subWeeks(anchor,1),{weekStartsOn:1}))},{label:"С начала года",from:`${year}-01-01`,to:moscowDayKey},...completedYtd,{label:"С начала месяца",from:toIso(currentMonthStart),to:moscowDayKey},{label:"Прошлый месяц",from:toIso(previousMonthStart),to:toIso(previousMonthEnd)},{label:"I кв.",from:toIso(startOfQuarter(new Date(year,0,1))),to:toIso(endOfQuarter(new Date(year,0,1)))} ,{label:"II кв.",from:toIso(startOfQuarter(new Date(year,3,1))),to:toIso(endOfQuarter(new Date(year,3,1)))},{label:"III кв.",from:toIso(startOfQuarter(new Date(year,6,1))),to:toIso(endOfQuarter(new Date(year,6,1)))},{label:"IV кв.",from:toIso(startOfQuarter(new Date(year,9,1))),to:toIso(endOfQuarter(new Date(year,9,1)))}]},[moscowDayKey,year]);
  ...
  return <Popover open={open} onOpenChange={next=>{if(next){setDraft(undefined);setHovered(undefined);setShowStandard(false)}setOpen(next)}}>
    ...
    <PopoverContent className="date-popover" align="end" side="bottom" sideOffset={10} collisionPadding={12} sticky="always">
      ...
      <Calendar className={showStandard?"calendar-standard-open":undefined} mode="single" locale={ru} selected={undefined} month={displayMonth} onMonthChange={setDisplayMonth} numberOfMonths={1} showOutsideDays={false} onDayClick={day=>pickDay(day)} onDayMouseLeave={()=>setHovered(undefined)} components={{DayButton:({day,modifiers,className,...props})=><CalendarDayButton day={day} modifiers={modifiers} className={[className,modifiers.preview?"range-preview-day":"",modifiers.hovered?"range-hover-day":"",modifiers.draftStart?"range-draft-start":"",modifiers.savedRange?"range-saved-day":""].filter(Boolean).join(" ")} {...props} onPointerUp={event=>pickTouchDay(event,day.date)} onMouseEnter={()=>setHovered(day.date)}/>}} modifiers={{...(savedRange?{savedRange}:{}),...(preview?{preview}:{}),...(hovered?{hovered}:{}),...(draft?.from?{draftStart:draft.from}:{})}} modifiersClassNames={{savedRange:"range-saved",preview:"range-preview",hovered:"range-hover",draftStart:"range-draft-start"}}/>
...
export function ExactDateControl({value="",onChange,title="ДАТА ЗАПИСИ",ariaLabel="Изменить дату записи",emptyLabel="Не указана"}:ExactDateProps){
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent className="date-popover fact-date-popover" align="start" side="top" sideOffset={8} collisionPadding={8} sticky="always">
      ...
      <Calendar mode="single" locale={ru} selected={undefined} month={displayMonth} onMonthChange={setDisplayMonth} numberOfMonths={1} showOutsideDays={false} onDayClick={day=>{onChange(toIso(day));setOpen(false);}} components={{DayButton:({day,modifiers,className,...props})=><CalendarDayButton day={day} modifiers={modifiers} className={[className,modifiers.singleSelected?"calendar-single-selected":""].filter(Boolean).join(" ")} {...props}/>}} modifiers={{singleSelected:selectedDate}} modifiersClassNames={{singleSelected:"calendar-single-selected"}}/>
    </PopoverContent>
  </Popover>;
}
