...
export function DateRangeControl({compact=false,value,onChange,title="ПЕРИОД АНАЛИЗА",ariaLabel="Изменить период анализа"}:Props){
  ...
  const shortcuts=useMemo<Shortcut[]>(()=>{const anchor=new Date(`${localDayKey}T12:00:00`);const previousMonthStart=startOfMonth(subMonths(anchor,1));const previousMonthEnd=endOfMonth(previousMonthStart);const currentWeekStart=startOfWeek(anchor,{weekStartsOn:1});const previousWeekStart=startOfWeek(subWeeks(anchor,1),{weekStartsOn:1});const completedYtd=anchor.getMonth()===0?[]:[{label:"Год до конца прошлого месяца",from:`${year}-01-01`,to:toIso(previousMonthEnd),wide:true}];return [{label:"Эта неделя",from:toIso(currentWeekStart),to:toIso(endOfWeek(anchor,{weekStartsOn:1}))},{label:"Прошлая неделя",from:toIso(previousWeekStart),to:toIso(endOfWeek(subWeeks(anchor,1),{weekStartsOn:1}))},...completedYtd,{label:"С начала года",from:`${year}-01-01`,to:localDayKey},{label:"Прошлый месяц",from:toIso(previousMonthStart),to:toIso(previousMonthEnd)},{label:"I кв.",from:toIso(startOfQuarter(new Date(year,0,1))),to:toIso(endOfQuarter(new Date(year,0,1)))} ,{label:"II кв.",from:toIso(startOfQuarter(new Date(year,3,1))),to:toIso(endOfQuarter(new Date(year,3,1)))},{label:"III кв.",from:toIso(startOfQuarter(new Date(year,6,1))),to:toIso(endOfQuarter(new Date(year,6,1)))},{label:"IV кв.",from:toIso(startOfQuarter(new Date(year,9,1))),to:toIso(endOfQuarter(new Date(year,9,1)))}]},[localDayKey,year]);
