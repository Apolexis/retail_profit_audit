...
export function DateRangeControl({compact=false,value,onChange,title="ПЕРИОД АНАЛИЗА",ariaLabel="Изменить период анализа"}:Props){
  ...
  const shortcuts=useMemo<Shortcut[]>(()=>{const anchor=new Date(`${localDayKey}T12:00:00`);const previousMonthStart=startOfMonth(subMonths(anchor,1));const previousMonthEnd=endOfMonth(previousMonthStart);const currentWeekStart=startOfWeek(anchor,{weekStartsOn:1});const previousWeekStart=startOfWeek(subWeeks(anchor,1),{weekStartsOn:1});const completedYtd=anchor.getMonth()===0?[]:[{group:"Быстрые" as const,label:"С начала года по завершенный месяц",from:`${year}-01-01`,to:toIso(previousMonthEnd)}];return [{group:"Быстрые",label:"Сегодня",from:toIso(anchor),to:toIso(anchor)},{group:"Быстрые",label:"Текущая неделя",from:toIso(currentWeekStart),to:toIso(endOfWeek(anchor,{weekStartsOn:1}))},{group:"Быстрые",label:"Прошлая неделя",from:toIso(previousWeekStart),to:toIso(endOfWeek(subWeeks(anchor,1),{weekStartsOn:1}))},...completedYtd,{group:"Быстрые",label:"С начала этого года",from:`${year}-01-01`,to:localDayKey},{group:"Месяцы",label:"Предыдущий месяц",from:toIso(previousMonthStart),to:toIso(previousMonthEnd)},{group:"Кварталы",label:"I квартал",from:toIso(startOfQuarter(new Date(year,0,1))),to:toIso(endOfQuarter(new Date(year,0,1)))},{group:"Кварталы",label:"II квартал",from:toIso(startOfQuarter(new Date(year,3,1))),to:toIso(endOfQuarter(new Date(year,3,1)))},{group:"Кварталы",label:"III квартал",from:toIso(startOfQuarter(new Date(year,6,1))),to:toIso(endOfQuarter(new Date(year,6,1)))},{group:"Кварталы",label:"IV квартал",from:toIso(startOfQuarter(new Date(year,9,1))),to:toIso(endOfQuarter(new Date(year,9,1)))}]},[localDayKey,year]);
