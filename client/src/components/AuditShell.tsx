...
export function AuditShell({title,kicker,children}:{title:string;kicker:string;children:ReactNode}){
  ...
  const clearActionFocus=(target:HTMLElement)=>window.requestAnimationFrame(()=>target.blur());
  ...
  return <div className="packet">
    ...
    {standalone&&<nav className="mobile-quick-nav" aria-label="Быстрые действия">
      ...
      <button type="button" onClick={event=>{clearActionFocus(event.currentTarget);window.location.reload()}} aria-label="Обновить данные"><RefreshCw size={16}/><span>Обновить</span></button>
    </nav>}
