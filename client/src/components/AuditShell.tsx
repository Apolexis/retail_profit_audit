...
const navSections=[
  ...
  {title:"ОПЕРАЦИИ",items:[["/revenue","23","Выручка","operations"]]},
...
recommendations["23"]={title:"Выручка: передавайте фактический день",text:"Наличные расходы должны быть разнесены по строкам и объяснены. Это операционный реестр, который не заменяет финансовый факт и не меняет P&L.",action:"Перед передачей проверьте итог и комментарии ко всем ненулевым расходам."};
...
export function AuditShell({title,kicker,children}:{title:string;kicker:string;children:ReactNode}){
  ...
  const visibleNav=navSections.map(section=>({...section,items:section.items.filter(([, , ,access])=>access==="operations"?(isAdmin||isSeller):isSeller?false:access===true?isAdmin:access==="price"?hasPriceAccess:true)})).filter(section=>section.items.length>0);
  ...
  const sectionIsActive=(items:readonly (readonly [string,string,string,boolean|"price"|"operations"])[])=>items.some(([href])=>href===location);
  const sectionIsOpen=(section:{title:string;items:readonly (readonly [string,string,string,boolean|"price"|"operations"])[]})=>Object.hasOwn(expandedSections,section.title)?Boolean(expandedSections[section.title]):sectionIsActive(section.items);
