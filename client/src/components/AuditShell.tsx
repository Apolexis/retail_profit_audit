...
export function AuditShell({title,kicker,children}:{title:string;kicker:string;children:ReactNode}){
  ...
  return <div className="packet">
    ...
    {standalone&&<nav className="mobile-quick-nav" aria-label="Быстрые действия">
      <button type="button" onClick={event=>{event.currentTarget.blur();window.history.length>1?window.history.back():setLocation("/")}} aria-label="Назад"><ArrowLeft size={16}/><span>Назад</span></button>
      <button type="button" onClick={event=>{event.currentTarget.blur();window.history.forward()}} aria-label="Вперёд"><ArrowRight size={16}/><span>Вперёд</span></button>
      <button type="button" onClick={event=>{event.currentTarget.blur();window.location.reload()}} aria-label="Обновить данные"><RefreshCw size={16}/><span>Обновить</span></button>
    </nav>}
