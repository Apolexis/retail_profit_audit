...
type NavAccess = boolean | "price" | "operations" | "revenue" | "transfer";
...
const navSections: readonly NavSection[] = [
  { title: "УПРАВЛЕНИЕ МАГАЗИНАМИ", groups: [{ items: [["/revenue", "23", "Выручка", "revenue"], ["/revenue-plan", "36", "План", "operations"], ["/stock-control", "24", "Остатки", "operations"], ["/stock-transfers", "32", "Перемещения", "transfer"], ["/shipment-receipts", "35", "Приёмка", "operations"], ["/inventory-control", "25", "Инвентаризации", "operations"], ["/catalog-control", "26", "Номенклатура", true], ["/warehouse-control", "27", "Склады", true], ["/print-settings", "31", "Настройки печати", true], ["/requests", "30", "Заявки", "operations"], ["/onec-import", "33", "Импорт 1С", true]] }] },
    { title: "АНАЛИЗ МАГАЗИНОВ", groups: [
      { title: "АНАЛИТИКА", items: [["/", "00", "Сводка", false], ["/months", "01", "Месяцы", false], ["/pricing", "02", "Цены", false], ["/expenses", "03", "Расходы", false], ["/inventory", "04", "Остатки", false], ["/stores", "05", "Магазины", false], ["/compare", "06", "Сравнить", false], ["/evotor-sales/products", "29", "Проданные товары", true], ["/evotor-sales/receipts", "34", "Чеки Эвотор", true]] },
    ...
    { title: "УПРАВЛЕНИЕ", items: [["/import", "12", "Импорт", true], ["/manage", "13", "База импорта", true]] },
  ] },
  ...
  { title: "УПРАВЛЕНИЕ", groups: [{ items: [["/notifications", "17", "Сигналы", "operations"], ["/reports", "18", "Отчеты", true], ["/access", "15", "Доступ", true], ["/history", "16", "Журнал", true]] }] },
];
...
recommendations["32"]={title:"Перемещения: проводите только подтвержденный остаток",text:"Черновик не меняет складской баланс. Проведение создает две неизменяемые записи — расход у отправителя и приход у получателя.",action:"Сверьте остаток, добавьте позиции и проведите перемещение только после проверки склада-отправителя."};
recommendations["33"]={title:"Импорт 1С: сначала сопоставление",text:"Срезы БМ/СРС и расходные накладные 1С ведутся отдельно. Несопоставленные строки остаются в карантине и не меняют остатки магазинов или финансовый факт.",action:"Проверьте пакет, подтвердите получателей и товары общего справочника, затем используйте срез только как read-only операционный источник."};
recommendations["34"]={title:"Чеки Эвотор: сначала состав",text:"Реестр показывает только уже нормализованные документы и товарные строки Эвотор. Номер показывается лишь когда Cloud API V2 действительно его передал; внутренние и фискальные идентификаторы не раскрываются.",action:"Начните с текущего дня, выберите магазин или всю сеть, затем откройте чек для сверки состава и агрегированных оплат."};
recommendations["35"]={title:"Приёмка: сначала фактический товар",text:"Накладная 1С сама по себе не меняет остаток магазина. Совпавший фактический приём проводится один раз; расхождение проводит руководитель или администратор после подтверждения.",action:"Откройте поступившую накладную, внесите фактически принятое количество по каждой строке и укажите причину расхождения."};
...
recommendations["26"]={title:"Номенклатура: только через рабочий справочник",text:"Каталог Эвотор читается только для сопоставленной точки. Новая позиция по умолчанию ставится в минутную очередь выгрузки Эвотор; внутреннюю себестоимость касса получает только при отдельном включенном флаге.",action:"Сначала выберите точку и проверьте ее каталог, затем подтверждайте или корректируйте рабочую номенклатуру."};
...
recommendations["30"]={title:"Заявки: сначала потребность, затем печать",text:"Сохраненная заявка помогает магазину собрать потребность без изменения остатков. До закрытия ее можно открыть и скорректировать; после закрытия строки становятся неизменяемым снимком.",action:"Добавьте товары и количество, сохраните заявку, затем распечатайте сохраненные или закрытые заявки по группе и категории."};
...
export function AuditShell({title,kicker,children}:{title:string;kicker:string;children:ReactNode}){
  ...
  const {theme,toggleTheme,demoMode,toggleDemoMode,rangeLabel,setSelectedStore,selectedStores,setSelectedStores}=useAudit();
  ...
  const visibleNav=navSections.map(section=>({...section,groups:section.groups.map(group=>({...group,items:group.items.filter(([, , ,access])=>access==="revenue"?(isAdmin||isSeller):access==="operations"?(isAdmin||isSeller||isManager):access==="transfer"?(isAdmin||isManager):(isSeller||isManager)?false:access===true?isAdmin:access==="price"?hasPriceAccess:true)})).filter(group=>group.items.length>0)})).filter(section=>section.groups.length>0);
  ...
  const analyticsSection=!['12','13','14','15','16','17','18','20','21','22','23','24','25','26','27','28','30','31','32','33','34','35','36'].includes(kicker.slice(0,2));
  const visibleAnalyticsStores=(availableStores.data??[]).filter(store=>!store.isHidden);
  const allAnalyticsStores=selectedStores.length===0;
  const analyticsStoreLabel=allAnalyticsStores?"Все магазины":selectedStores[0]??"Все магазины";
  ...
  return <div className={standalone?"packet pwa-standalone":"packet"} data-demo-mode={demoMode?"true":"false"}>
    ...
    <main className="packet-main">{analyticsSection&&<section className={`analysis-filter analysis-global-slice${kicker.startsWith("00")?" summary-period-filter":""}`}><div className="analysis-filter-copy"><span>ОБЩИЙ СРЕЗ</span><strong>{rangeLabel}</strong><small>Применяется ко всем графикам, сравнениям и итогам текущего раздела.</small></div><DateRangeControl/><label className="analysis-store-scope"><span>МАГАЗИНЫ</span><ThemedSelect value={allAnalyticsStores?"":selectedStores[0]??""} aria-label="Выбрать магазин для общего аналитического среза" onChange={event=>setSelectedStores(event.target.value?[event.target.value]:[])}><option value="">Все магазины</option>{visibleAnalyticsStores.map(store=><option key={store.id} value={store.name}>{store.name}</option>)}</ThemedSelect></label></section>}{children}<aside className="section-recommendation"><span>УПРАВЛЕНЧЕСКИЙ ФОКУС</span><div><h3>{guidance.title}</h3><p>{guidance.text}</p></div><strong>{guidance.action}</strong></aside></main>
