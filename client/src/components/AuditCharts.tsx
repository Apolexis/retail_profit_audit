...
export function BenchmarkBars({ data, median, mode="percent", selectedLabel }: {data:{store:string;value:number;selected:boolean}[];median:number;mode?:TooltipMode;selectedLabel:string}) {
  ...
  return <div className="benchmark-chart"><div className="median-badge"><i/> Медиана сети: <b>{chartTick(median,mode)}</b> · {selectedLabel} — золотой маркер</div><ResponsiveContainer width="100%" height={height}><BarChart data={data} layout="vertical" margin={{top:16,right:30,left:18,bottom:14}} barCategoryGap={6}><CartesianGrid horizontal={false} stroke="#3a2031"/><XAxis type="number" tickFormatter={value=>chartTick(value,mode)} axisLine={false} tickLine={false} stroke="#ab90a0"/><YAxis type="category" dataKey="store" width={68} tick={{fontSize:11,fill:"#d9c7d0"}} axisLine={false} tickLine={false}/><Tooltip content={<TinyTooltip mode={mode}/>}/><ReferenceLine x={median} stroke="#ff597c" strokeWidth={2} strokeDasharray="5 5"/><Bar dataKey="value" name="Значение" radius={[0,6,6,0]} isAnimationActive animationDuration={650}>{data.map(row=><Cell key={row.store} fill={row.selected?"#ffc15e":"#762c4b"}/>)}</Bar></BarChart></ResponsiveContainer></div>
}
