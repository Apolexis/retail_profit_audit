...
export function MetricLineChart({data,lines,percent=false,unit="k",displayMode}:{data:readonly Record<string,unknown>[];lines:MetricLine[];percent?:boolean;unit?:"k"|"m";displayMode?:MetricChartTooltipMode}){
  ...
  // Сначала рисуем крупные ряды. Меньшие ряды идут поверх и не теряются за общим силуэтом.
  ...
  return <div className="metric-chart">
    <div className="chart-toolbar">
      <div className="chart-series-control" aria-label="Управление рядами графика">
        {activeLines.map((line,index)=>{const muted=hidden.includes(line.key);return <button type="button" aria-pressed={!muted} key={line.key} onClick={()=>toggle(line.key)} className={muted?"series-toggle muted":"series-toggle"}><i style={{background:colorFor(line,index)}}/>{line.name}</button>;})}
      </div>
      {data.length>1&&<ChartViewControls view={chartView} onChange={setRequestedView}/>} 
    </div>
    <ResponsiveContainer width="100%" height={chartLayout==="single-period-comparison"?Math.max(300,singleRows.length*44+48):306}>
      {chartLayout==="single-period-comparison"?
        <BarChart data={singleRows} layout="vertical" margin={{top:16,right:32,left:12,bottom:10}} barCategoryGap={10}>
          <CartesianGrid horizontal={false} stroke={palette.grid}/><XAxis type="number" domain={[singleTicks[0],singleTicks.at(-1)??0]} ticks={singleTicks} axisLine={false} tickLine={false} stroke={palette.axis} tick={{fill:palette.tick,fontSize:11}} tickFormatter={value=>chartTick(value,mode)}/><YAxis type="category" dataKey="label" width={112} axisLine={false} tickLine={false} tick={{fill:palette.tick,fontSize:11}}/><ReferenceLine x={0} stroke={palette.axis} strokeOpacity={.78} strokeWidth={1.2}/><Tooltip cursor={false} wrapperStyle={{pointerEvents:"none",zIndex:20}} content={<TinyTooltip mode={mode}/>}/><Bar dataKey="value" name="Значение" radius={[0,6,6,0]} maxBarSize={28} isAnimationActive animationDuration={650}>{singleRows.map(row=><Cell key={row.label} fill={row.color}/>)}</Bar>
        </BarChart>
      :chartView==="bar"?
        <BarChart data={[...data]} margin={{top:18,right:32,left:20,bottom:10}} barCategoryGap={data.length===1?"34%":"18%"}>
          <CartesianGrid vertical={false} stroke={palette.grid}/><XAxis dataKey="month" axisLine={false} tickLine={false} stroke={palette.axis}/><YAxis domain={[ticks[0],ticks.at(-1)??0]} ticks={ticks} axisLine={false} tickLine={false} width={78} stroke={palette.axis} tick={{fill:palette.tick,fontSize:11}} tickFormatter={value=>chartTick(value,mode)}/><ReferenceLine y={0} stroke={palette.axis} strokeOpacity={.78} strokeWidth={1.2}/><Tooltip cursor={cursor} wrapperStyle={{pointerEvents:"none",zIndex:20}} content={<TinyTooltip mode={mode}/>}/>{visibleLines.map((line,index)=><Bar key={line.key} dataKey={line.key} name={line.name} fill={colorFor(line,index)} radius={[6,6,0,0]} maxBarSize={data.length===1?86:undefined} isAnimationActive animationDuration={650}/>)}</BarChart>
      :chartView==="overlay"?
        <BarChart data={[...data]} margin={{top:18,right:32,left:20,bottom:10}} barCategoryGap="30%" barGap={0}>
      ...
      :
        <AreaChart data={[...data]} margin={{top:18,right:32,left:20,bottom:10}}>
