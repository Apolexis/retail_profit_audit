...
describe("базовые управляющие элементы", () => {
  ...
  it("добавляет общий компактный выбор одного магазина рядом с периодом", () => {
    expect(shell).toContain("selectedStores,setSelectedStores");
    ...
    expect(shell).toContain('aria-label="Выбрать магазин для общего аналитического среза"');
    expect(shell).not.toContain("toggleAnalyticsStore");
    expect(css).toContain(".packet .analysis-global-slice");
    expect(css).toContain(".packet .analysis-store-scope > select");
  });
