...
// Stock and catalog own their data-aware sort state in React. Hydrating those
// card-mode tables as generic DOM tables recreates visual sort affordances for
// hidden headers at intermediate widths, even though a heading click already
// performs the correct server/client sort.
const tableSelector = "table.data-table:not(.stock-table):not(.catalog-table)";
...
export function SortableTablesBootstrap() {
  useEffect(() => {
    ...
    const hydrate = () => {
      // These two tables keep the authoritative sort in React. Remove residual
      // attributes left by a hot update before the generic bootstrap was
      // narrowed, so card-mode hidden headers cannot paint orphan arrows.
      document.querySelectorAll<HTMLTableCellElement>("table.stock-table th, table.catalog-table th").forEach(header => {
        delete header.dataset.sortable;
        delete header.dataset.sortDirection;
        header.classList.remove("sortable-column", "numeric-column", "sort-active");
      });
      ...
      document.querySelectorAll<HTMLElement>(".data-table-wrap").forEach(wrap => {
        if (wrap.dataset.dragScroll === "false") {
          const cleanup = dragCleanups.get(wrap);
          if (cleanup) {
            cleanup();
