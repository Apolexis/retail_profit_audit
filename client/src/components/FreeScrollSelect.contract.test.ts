...
describe("немодальный тематичный select", () => {
  ...
  it("сохраняет доступную роль списка, выбранный пункт и групповые заголовки", () => {
    ...
    expect(styles).toContain('@media (hover: hover) and (pointer: fine) { .free-scroll-select-edge.is-active { display: flex; }');
	  ...
	  it("позволяет рабочим длинным селектам включать встроенный поиск без отдельного поля", () => {
	    const themed = readFileSync(new URL("./ui/themed-select.tsx", import.meta.url), "utf8");
	    expect(themed).toContain("searchable?: boolean;");
	    expect(themed).toContain("searchPlaceholder?: string;");
	    expect(themed).toContain("searchPlaceholder={searchPlaceholder}");
	    expect(themed).toContain("searchable={Boolean(searchable && options.length > 10)}");
	    expect(component).not.toContain("searchInputRef.current?.focus");
	    expect(component).toContain("onOpenAutoFocus={event => event.preventDefault()}");
	  });

	  it("не перекрывает выбранное значение строкой поиска", () => {
	    expect(component).toContain('data-slot="select-value"');
    expect(styles).toContain('.free-scroll-select-trigger [data-slot="select-value"] { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }');
    expect(component).toContain('className="free-scroll-select-search"');
	    expect(styles).toContain(".free-scroll-select-search { display: block; padding: 2px 2px 6px; }");
	  });

	  it("находит товар по нескольким частям названия, коду или их сочетанию", () => {
	    expect(component).toContain("const searchTokens = (value: string)");
	    expect(component).toContain('.replace(/ё/g, "е")');
	    expect(component).toContain("return normalizedSearchTokens.every(token => label.includes(token));");
	  });
});
