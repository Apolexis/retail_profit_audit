...
describe("начальный пароль магазина", () => {
  ...
  it("ограничивает руководителя операционным контуром", () => {
    expect(app).toContain('const managerPaths = new Set(["/revenue-plan", "/stock-control", "/stock-transfers", "/shipment-receipts", "/inventory-control", "/requests", "/notifications", "/profile"])');
  ...
  it("регистрирует отдельные маршруты остатков, перемещений, приемки, ревизии, номенклатуры и заявок", () => {
    expect(app).toContain('<Route path="/stock-control" component={StockControl} />');
    expect(app).toContain('<Route path="/stock-transfers" component={StockTransfers} />');
    expect(app).toContain('<Route path="/shipment-receipts" component={ShipmentReceipts} />');
    ...
    expect(app).toContain('<Route path="/revenue-plan" component={RevenuePlan} />');
  });
});
