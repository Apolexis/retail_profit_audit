...
it("сохраняет утвержденное название приложения в окружении и тематических PWA manifest", () => {
  ...
  expect(auditContext).toContain('new CustomEvent("audit-pwa-theme-change"');
	  ...
	  expect(auditContext).toContain('window.addEventListener("beforeprint", setWhitePrintSurface);');
	  expect(auditContext).toContain('root.style.setProperty("background", "#ffffff", "important");');
	  expect(auditContext).toContain('window.addEventListener("afterprint", restorePwaSurface);');
