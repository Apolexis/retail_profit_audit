...
const weightedImportedPercent=(rows:ImportedPeriod[],metricCode:string,weight:(row:ImportedPeriod)=>number)=>{const points=rows.map(row=>({value:Number(row.metrics[metricCode]),weight:Math.max(0,weight(row))})).filter(point=>Number.isFinite(point.value));if(!points.length)return 0;const totalWeight=points.reduce((total,point)=>total+point.weight,0);return totalWeight>0?points.reduce((total,point)=>total+point.value*point.weight,0)/totalWeight:points.reduce((total,point)=>total+point.value,0)/points.length;};
