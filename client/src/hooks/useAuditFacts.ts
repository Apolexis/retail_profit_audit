...
const markupPercent=(sales:number,purchase:number)=>purchase?((sales-purchase)/purchase*100):0;

const isInsideRange = (entryDate: string, range: { from: string; to: string }) => entryDate >= range.from && entryDate <= range.to;

export function useAuditFacts(options:{includeHidden?:boolean;comparisonRange?:{from:string;to:string}}={}){
  const imported=useImportedAudit(options.comparisonRange,options.includeHidden===true);
  ...
  const periods=useMemo(()=>prepareDailyFacts(imported.periods.filter(row=>isInsideRange(row.entryDate,range)),range),[imported.periods,range.from,range.to]);
  const comparisonPeriods=useMemo(()=>options.comparisonRange?prepareDailyFacts(imported.periods.filter(row=>isInsideRange(row.entryDate,options.comparisonRange!)),options.comparisonRange):[],[imported.periods,options.comparisonRange?.from,options.comparisonRange?.to]);
  const storeNames=useMemo(()=>Array.from(new Set(periods.map(row=>row.store))).sort(),[periods]);
  const summaryFor=useCallback((rows:ImportedPeriod[],store:string,storeId?:number):FactSummary=>{const revenue=sum(rows,"revenue"),cashRevenue=sum(rows,"cash_revenue"),cashlessRevenue=sum(rows,"cashless_revenue"),receiptsTotal=sum(rows,"receipts_total"),grossProfit=sum(rows,"gross_profit"),netProfit=sum(rows,"net_profit"),purchases=sum(rows,"purchases"),purchaseSmoked=sum(rows,"purchase_smoked"),purchaseFrozen=sum(rows,"purchase_frozen"),salesSmoked=sum(rows,"sales_smoked"),salesFrozen=sum(rows,"sales_frozen"),writeoffSmoked=sum(rows,"writeoff_smoked"),writeoffFrozen=sum(rows,"writeoff_frozen"),stockOpen=rows.length?boundaryStock(rows,"stock_open","first"):0,stockClose=rows.length?boundaryStock(rows,"stock_close","last"):0,expenseByCode=Object.fromEntries(expenseDefinitions.map(([code])=>[code,sum(rows,code)])),expenses=expenseDefinitions.reduce((total,[code])=>total+Math.abs(expenseByCode[code]),0),markupSmoked=markupPercent(salesSmoked,purchaseSmoked),markupFrozen=markupPercent(salesFrozen,purchaseFrozen),markupTotal=markupPercent(salesSmoked+salesFrozen,purchaseSmoked+purchaseFrozen);return {store,storeId,revenue,cashRevenue,cashlessRevenue,receiptsTotal,grossProfit,netProfit,purchases,purchaseSmoked,purchaseFrozen,salesSmoked,salesFrozen,writeoffSmoked,writeoffFrozen,stockOpen,stockClose,expenses,netMargin:revenue?netProfit/revenue*100:0,grossMargin:revenue?grossProfit/revenue*100:0,markup:markupTotal,markupSmoked,markupFrozen,markupTotal,coverDays:coverageDays(stockClose,revenue,rangeDays),expenseByCode};},[rangeDays]);
  ...
  const comparisonRowsFor=useCallback((store:string)=>store==="__all__"?comparisonPeriods:comparisonPeriods.filter(row=>row.store===store),[comparisonPeriods]);
  ...
  return {available:periods.length>0,hasAnyFinancialFacts:imported.hasAnyFinancialFacts,financialCoverage:imported.financialCoverage,financialCoverageLoading:imported.financialCoverageLoading,loading:imported.loading,error:imported.error,periods,comparisonPeriods,storeNames,summaries,network,rowsFor,comparisonRowsFor,summaryFor,monthlyFor,medianFor,range};
}
